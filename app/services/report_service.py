...
依赖关系:
    ...
    - app.services._report_defect_builder: 缺陷维度构建器基类

设计说明:
    采用静态方法设计，服务不持有状态，数据库会话由调用方传入。
    报告生成流程为：查询结果 -> 统计计算 -> 内容构建 -> 持久化。
    统计指标包括总数、通过数、失败数、阻塞数和通过率。
    缺陷维度构建逻辑拆分至 _ReportDefectDimensionBuilder 基类。
"""
...
from app.services._report_defect_builder import _ReportDefectDimensionBuilder


class ReportService(_ReportDefectDimensionBuilder):
    ...
    设计意图:
        ...
        缺陷维度构建逻辑继承自 _ReportDefectDimensionBuilder。
    """
