"""测试用例批量生成 - 模块历史失败归因子 Mixin。

从 batch_mixin.py 拆分，承载：
- _preload_module_failure_feedback: 批量预加载各模块历史执行失败类型（R4 修复，消除 N+1）
- _refine_context_for_point: 查询同模块历史失败归因并注入生成上下文（Task 12）
"""
...
class BatchModuleFailureFeedbackMixin:
    """模块历史失败归因子 Mixin。

    依赖聚合类提供 self.db（来自 BaseMixin）。
    """
...
__all__ = ["BatchModuleFailureFeedbackMixin"]
