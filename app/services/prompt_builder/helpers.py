...
# Trigger condition auto-inference templates
...
def _infer_condition(edge: Dict[str, Any], edge_type: str, target_node: Dict[str, Any], source_node: Optional[Dict[str, Any]] = None) -> str:
    ...
    template = _CONDITION_TEMPLATES.get(edge_type, '未指定')
    inferred = template.format(source_name=source_name, target_name=target_name)
    logger.info(f"Auto-inferred {edge_type} condition: {inferred} (edge={edge.get('id', '?')})")
...
def _group_edges_by_source(edge_list: List[Dict[str, Any]]) -> Dict[int, List[Dict[str, Any]]]:
    """按源节点 screen_id 分组连线。
