...
async def get_dashboard_overview(
    ...
    def _overview(sync_db):
        try:
            ...
            total_runs = sync_db.query(func.count(PipelineRun.id)).filter(PipelineRun.started_at >= since)
            completed_runs = sync_db.query(func.count(PipelineRun.id)).filter(PipelineRun.started_at >= since, PipelineRun.status == "completed")
            total_tokens = sync_db.query(
            ...
            total_cost = sync_db.query(func.coalesce(func.sum(AICallLog.cost_usd), 0)).filter(AICallLog.created_at >= since)
            avg_duration = sync_db.query(func.coalesce(func.avg(duration_seconds(PipelineRun.started_at, PipelineRun.finished_at, sync_db)), 0)).filter(
            ...
            cache_hit_steps = sync_db.query(func.count(PipelineStep.id)).filter(
                PipelineStep.cache_key.isnot(None), PipelineStep.status == "skipped", PipelineStep.started_at >= since,
            )
            total_steps = sync_db.query(func.count(PipelineStep.id)).filter(PipelineStep.started_at >= since)
