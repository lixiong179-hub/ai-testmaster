...
async def create_test_data(
    ...
    try:
        test_data = service.create_test_data(**request.model_dump())
...
async def get_test_data(
    ...
    """获取单个测试数据"""
    service = TestDataService(db)
    test_data = service.get_test_data(test_data_id)
    if not test_data:
        raise HTTPException(
...
async def get_test_data_by_step(
    ...
    """获取步骤的所有测试数据"""
    service = TestDataService(db)
    data_list = service.get_test_data_by_step(step_id)
    return {
        "step_id": step_id,
...
async def update_test_data(
    ...
    test_data = service.update_test_data(test_data_id, **update_data)
    if not test_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="测试数据不存在"
...
async def delete_test_data(
    ...
    """删除测试数据"""
    service = TestDataService(db)
    success = service.delete_test_data(test_data_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="测试数据不存在"
...
async def generate_step_data(
    ...
    service = TestDataService(db)
    try:
        generated_data = service.generate_step_data(step_id)
        return {
            "step_id": step_id,
...
async def auto_generate_test_data(
    ...
    service = TestDataService(db)
    try:
        created_list = service.auto_generate_for_step(step_id, action_description)
        return {
            "success": True,
