...
本模块实现 AIClient Protocol，封装 OpenAI SDK 调用逻辑：
    ...
    - AsyncOpenAIClient：异步版本，定义于 _openai_async_client.py，
      在本模块底部 re-export 以保持 `from app.ai.openai_client import AsyncOpenAIClient`
      的向后兼容
...
# Re-export AsyncOpenAIClient 以保持向后兼容：
#     `from app.ai.openai_client import AsyncOpenAIClient`
# 实际定义位于 app.ai._openai_async_client，便于单文件 ≤350 行的拆分。
from app.ai._openai_async_client import AsyncOpenAIClient  # noqa: E402
