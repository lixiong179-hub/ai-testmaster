...
�?M3 阶段 Pipeline 质量回归�?M4 先验/后验质量分验证提供基准数据集�?
...
| 项目 | 目录 | 场景 | 输入信号 | 标注内容 |
|---|---|---|---|---|
| Alpha | `project_alpha/` | 1（全输入新项目） | PRD + 测试�?+ UI | 预期用例 |
| Beta | `project_beta/` | 4（旧项目双向扫描�?| 50 历史用例 + �?PRD + UI 变更 | 预期 verdicts |
...
| `prd.json` | PRD 需求描述，按模块分�?|
| `test_points.json` | 测试点列�?|
| `ui_prototype.json` | UI 原型屏信�?|
| `expected_cases.json` | 预期生成的测试用�?|
...
| `new_prd.json` | 新的 PRD 需求变�?|
| `ui_changes.json` | UI 变更描述 |
| `expected_verdicts.json` | 50 条预�?verdict 标注 |
...
| `project.json` | 项目元数�?|
| `ui_prototype.json` | UI 原型屏信�?|
| `expected_inference.json` | 预期的反推业务能�?|
