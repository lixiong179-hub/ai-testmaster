...
class TestRequirementLinkAPI:
    ...
    def test_list_without_auth(self, client):
        resp = client.get("/api/v1/requirement-link/list/1")
        assert resp.status_code in (401, 403, 404)

    def test_get_without_auth(self, client):
        resp = client.get("/api/v1/requirement-link/1")
        assert resp.status_code in (401, 403, 404)
    ...
    def test_delete_without_auth(self, client):
        resp = client.delete("/api/v1/requirement-link/1")
        assert resp.status_code in (401, 403, 404)

    def test_list_nonexistent_project(self, client, authHeaders):
        resp = client.get(
