...
覆盖范围�?
- �?main_order 时按 main_order 排序
- �?main_order 时回退�?screen_order
- main 节点在前，branch 在后
- main_order 优先�?screen_order
"""
...
class MockNode:
    """模拟FlowNodeSchema对象，支持main_order属�?""
...
class TestSortFlowNodes:
    ...
    def test_sort_with_main_order(self) -> None:
        ...
        nodes = [
            MockNode(1, 3, '第三�?, 'main', main_order=3),
            MockNode(2, 1, '第一�?, 'main', main_order=1),
            MockNode(3, 2, '第二�?, 'main', main_order=2),
        ]

        result = _sort_flow_nodes(nodes)

        assert result[0].screen_name == '第一�?
        assert result[1].screen_name == '第二�?
        assert result[2].screen_name == '第三�?

    def test_sort_without_main_order_fallback(self) -> None:
        """测试无main_order时回退到screen_order排序"""
        nodes = [
            MockNode(1, 3, '第三�?, 'main', main_order=None),
            MockNode(2, 1, '第一�?, 'main', main_order=None),
            MockNode(3, 2, '第二�?, 'main', main_order=None),
        ]
    ...
    def test_sort_main_order_priority_over_screen_order(self) -> None:
        ...
        # main_order=1 应排�?main_order=2 前面，尽�?screen_order 相反
