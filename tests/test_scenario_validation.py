...
class TestScenarioQueryManagement:
    ...
    def test_query_main_flow(self):
        """验证查询功能主干流程"""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '查询�?, 'ui_spec_elements': [{'type': 'input', 'label': '关键�?}, {'type': 'button', 'label': '查询'}]},
            {'screen_id': 2, 'screen_order': 2, 'flow_type': 'main', 'screen_name': '结果列表', 'ui_spec_elements': [{'type': 'table', 'label': '数据列表'}, {'type': 'button', 'label': '导出'}]},
            {'screen_id': 3, 'screen_order': 3, 'flow_type': 'main', 'screen_name': '详情�?, 'ui_spec_elements': [{'type': 'text', 'label': '详细信息'}]},
        ]
        edges = [
        ...
        prompt = PromptBuilder.build_graph_prompt(
            ...
            module_info={'name': '查询管理', 'description': '支持多条件组合查�?},
            requirement_content='用户可以通过关键词查询数�?, test_point_json='{}', ui_specs_text=''
        )
    ...
    def test_query_branch_flow(self):
        """验证查询功能分支流程（高级筛选）"""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '查询�?, 'ui_spec_elements': [{'type': 'button', 'label': '高级筛�?}]},
            {'screen_id': 2, 'screen_order': 2, 'flow_type': 'branch', 'screen_name': '高级筛选弹�?, 'ui_spec_elements': [{'type': 'input', 'label': '时间范围'}, {'type': 'select', 'label': '状�?}]},
        ]
        edges = [
            {'source': '1', 'target': '2', 'edge_type': 'branch', 'condition': '用户点击高级筛选按�?, 'label': '条件分支'},
        ]
        prompt = PromptBuilder.build_graph_prompt(
            nodes=nodes, edges=edges, module_info=None, requirement_content='', test_point_json='{}', ui_specs_text=''
        )
        assert '分支流程' in prompt
        assert '用户点击高级筛选按�? in prompt
        assert '高级筛选弹�? in prompt

    def test_query_exception_flow(self):
        """验证查询功能异常流程（非法输入）"""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '查询�?, 'ui_spec_elements': [{'type': 'input', 'label': '关键�?}]},
            {'screen_id': 2, 'screen_order': 2, 'flow_type': 'exception', 'screen_name': '错误提示', 'ui_spec_elements': [{'type': 'text', 'label': '请输入有效字�?}]},
        ]
        edges = [
            {'source': '1', 'target': '2', 'edge_type': 'exception', 'condition': '输入非法字符或超长文�?, 'label': '异常跳转'},
        ]
        prompt = PromptBuilder.build_graph_prompt(
            nodes=nodes, edges=edges, module_info=None, requirement_content='', test_point_json='{}', ui_specs_text=''
        )
        assert '异常流程' in prompt
        assert '输入非法字符或超长文�? in prompt
...
class TestScenarioECommerceOrder:
    ...
    def test_order_full_flow(self):
        ...
        nodes = [
            ...
            {'screen_id': 3, 'screen_order': 3, 'flow_type': 'branch', 'screen_name': '优惠券选择', 'ui_spec_elements': [{'type': 'select', 'label': '可用优惠�?}]},
            {'screen_id': 4, 'screen_order': 4, 'flow_type': 'exception', 'screen_name': '库存不足提示', 'ui_spec_elements': [{'type': 'text', 'label': '商品库存不足'}]},
        ]
        edges = [
            {'source': '1', 'target': '2', 'edge_type': 'normal', 'condition': '', 'label': '正常流转'},
            {'source': '2', 'target': '3', 'edge_type': 'branch', 'condition': '用户选择使用优惠�?, 'label': '条件分支'},
        ...
        assert '分支 A: 从步�?2 分支' in prompt
        assert '用户选择使用优惠�? in prompt
        assert '异常 A: 从步�?2 异常跳转' in prompt
...
class TestScenarioUserRegistration:
    ...
    def test_registration_with_bypass(self):
        """验证注册流程（含旁路-协议弹窗�?""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '注册�?, 'ui_spec_elements': [{'type': 'input', 'label': '用户�?}, {'type': 'input', 'label': '密码'}]},
            {'screen_id': 2, 'screen_order': 2, 'flow_type': 'exception', 'screen_name': '用户名重复提�?, 'ui_spec_elements': [{'type': 'text', 'label': '用户名已存在'}]},
            {'screen_id': 3, 'screen_order': 3, 'flow_type': 'bypass', 'screen_name': '用户协议弹窗', 'ui_spec_elements': [{'type': 'text', 'label': '请同意用户协�?}, {'type': 'button', 'label': '同意'}]},
        ]
        edges = [
...
class TestScenarioFileUpload:
    ...
    def test_upload_exception_flows(self):
        """验证文件上传异常流程（格式错�?超大文件�?""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '上传�?, 'ui_spec_elements': [{'type': 'button', 'label': '选择文件'}, {'type': 'button', 'label': '上传'}]},
        ...
        assert '异常 A: 从步�?1 异常跳转' in prompt
        assert '上传非PDF格式文件' in prompt
        assert '异常 B: 从步�?1 异常跳转' in prompt
...
class TestScenarioApprovalProcess:
    ...
    def test_approval_complex_flow(self):
        """验证审批流程（主�?分支+旁路�?""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '待审批列�?, 'ui_spec_elements': [{'type': 'table', 'label': '审批列表'}, {'type': 'button', 'label': '审批'}]},
            {'screen_id': 2, 'screen_order': 2, 'flow_type': 'main', 'screen_name': '审批详情', 'ui_spec_elements': [{'type': 'button', 'label': '通过'}, {'type': 'button', 'label': '驳回'}]},
            {'screen_id': 3, 'screen_order': 3, 'flow_type': 'branch', 'screen_name': '驳回重审�?, 'ui_spec_elements': [{'type': 'input', 'label': '驳回原因'}]},
            {'screen_id': 4, 'screen_order': 4, 'flow_type': 'bypass', 'screen_name': '催办提醒弹窗', 'ui_spec_elements': [{'type': 'text', 'label': '您有3条待�?}]},
        ]
        edges = [
            {'source': '1', 'target': '2', 'edge_type': 'normal', 'condition': '', 'label': '正常流转'},
            {'source': '2', 'target': '3', 'edge_type': 'branch', 'condition': '审批人点击驳回按�?, 'label': '条件分支'},
            {'source': '1', 'target': '4', 'edge_type': 'bypass', 'condition': '进入页面时自动弹�?, 'label': '旁路步骤'},
        ]
        prompt = PromptBuilder.build_graph_prompt(
            nodes=nodes, edges=edges,
            module_info={'name': '审批管理', 'description': '支持多级审批和驳回重�?},
            requirement_content='审批流程支持通过、驳回、催�?, test_point_json='{}', ui_specs_text=''
        )
        assert '步骤 1: [截图1 - 待审批列表]' in prompt
        assert '步骤 2: [截图2 - 审批详情]' in prompt
        assert '分支 A: 从步�?2 分支' in prompt
        assert '审批人点击驳回按�? in prompt
        assert '旁路 A: 进入步骤 1 �? in prompt
...
class TestScenarioSystemLogin:
    ...
    def test_login_all_flow_types(self):
        """验证登录流程（主�?异常+旁路�?""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '登录�?, 'ui_spec_elements': [{'type': 'input', 'label': '用户�?}, {'type': 'input', 'label': '密码'}, {'type': 'button', 'label': '登录'}]},
            ...
            {'screen_id': 4, 'screen_order': 4, 'flow_type': 'bypass', 'screen_name': '验证码弹�?, 'ui_spec_elements': [{'type': 'input', 'label': '验证�?}, {'type': 'button', 'label': '确认'}]},
        ]
        edges = [
            {'source': '1', 'target': '2', 'edge_type': 'exception', 'condition': '输入错误密码', 'label': '异常跳转'},
            {'source': '1', 'target': '3', 'edge_type': 'exception', 'condition': '连续5次登录失�?, 'label': '异常跳转'},
            {'source': '1', 'target': '4', 'edge_type': 'bypass', 'condition': '首次登录或IP变更时弹�?, 'label': '旁路步骤'},
        ]
        prompt = PromptBuilder.build_graph_prompt(
            nodes=nodes, edges=edges,
            module_info={'name': '系统登录', 'description': '支持用户名密码登录和验证码校�?},
            requirement_content='登录功能需要验证码校验和安全锁�?, test_point_json='{}', ui_specs_text=''
        )
        assert '步骤 1: [截图1 - 登录页]' in prompt
        assert '异常 A: 从步�?1 异常跳转' in prompt
        assert '输入错误密码' in prompt
        assert '异常 B: 从步�?1 异常跳转' in prompt
        assert '连续5次登录失�? in prompt
        assert '旁路 A: 进入步骤 1 �? in prompt
        assert '验证码弹�? in prompt
...
class TestPromptQualityMetrics:
    ...
    def test_prompt_structure_completeness(self):
        """验证Prompt结构完整�?""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '测试�?, 'ui_spec_elements': [{'type': 'button', 'label': '提交'}]},
        ]
        edges = []
        prompt = PromptBuilder.build_graph_prompt(
            nodes=nodes, edges=edges,
            module_info={'name': '测试模块', 'description': '测试描述'},
            requirement_content='测试需�?, test_point_json='{"point": "测试�?}', ui_specs_text='UI规格'
        )
        # 验证所有必要部分存�?
        assert '资深测试工程�? in prompt
        assert '测试点信�? in prompt
    ...
    def test_prompt_ui_elements_description(self):
        """验证UI元素描述准确�?""
        nodes = [
            {'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '表单�?, 'ui_spec_elements': [
                {'type': 'input', 'label': '用户�?},
                {'type': 'input', 'label': '邮箱'},
    ...
    def test_generation_rules_count(self):
        """验证生成要求数量"""
        nodes = [{'screen_id': 1, 'screen_order': 1, 'flow_type': 'main', 'screen_name': '�?, 'ui_spec_elements': []}]
        prompt = PromptBuilder.build_graph_prompt(
