...
class TestUserServiceCreate:
    ...
    def test_create_user_password_hashed(self, db):
        userCreate = UserCreate(
            username=f"svc_hashed_{uuid.uuid4().hex[:8]}",
            email=f"svc_hashed_{uuid.uuid4().hex[:8]}@test.com",
            password="Test@123456",
        )
        user = UserService.create_user(db, userCreate)
        assert pwd_context.verify("Test@123456", user.password_hash) is True
...
class TestUserServiceUpdate:
    ...
    def test_update_user_partial_fields(self, db):
        userCreate = UserCreate(
            username=f"svc_partupd_{uuid.uuid4().hex[:8]}",
            email=f"svc_partupd_{uuid.uuid4().hex[:8]}@test.com",
            password="Test@123456",
            phone="13800138000",
        )
        user = UserService.create_user(db, userCreate)
        userUpdate = UserUpdate(phone="13900139000")
        updated = UserService.update_user(db, user.id, userUpdate)
        assert updated.phone == "13900139000"
        assert updated.email == userCreate.email
...
class TestUserServiceDelete:
    ...
    def test_delete_user_cleans_role_associations(self, db):
        userCreate = UserCreate(
            username=f"svc_delrole_{uuid.uuid4().hex[:8]}",
            email=f"svc_delrole_{uuid.uuid4().hex[:8]}@test.com",
            password="Test@123456",
        )
        user = UserService.create_user(db, userCreate)
        roleCreate = RoleCreate(
            name=f"svc_del_assign_{uuid.uuid4().hex[:8]}",
            desc="to be cleaned",
        )
...
class TestRoleService:
    ...
    def test_update_role_permissions(self, db):
        roleCreate = RoleCreate(
            name=f"svc_permrole_{uuid.uuid4().hex[:8]}",
            desc="perm update",
            permissions=["read"],
        )
        role = RoleService.create_role(db, roleCreate)
        roleUpdate = RoleUpdate(permissions=["read", "write", "delete"])
        updated = RoleService.update_role(db, role.id, roleUpdate)
        assert updated.permissions == ["read", "write", "delete"]
...
class TestUserRoleService:
    ...
    def test_get_user_roles(self, db):
        ...
        user = UserService.create_user(db, userCreate)
        roleCreate = RoleCreate(
            name=f"svc_listrole_{uuid.uuid4().hex[:8]}",
            desc="list test",
            permissions=["read"],
        )
        role = RoleService.create_role(db, roleCreate)
    ...
    def test_get_user_roles_empty(self, db):
        userCreate = UserCreate(
            username=f"svc_emptyroles_{uuid.uuid4().hex[:8]}",
            email=f"svc_emptyroles_{uuid.uuid4().hex[:8]}@test.com",
            password="Test@123456",
        )
        user = UserService.create_user(db, userCreate)
        roles = UserRoleService.get_user_roles(db, user.id)
...
class TestRBACService:
    ...
    def test_check_permission_role_without_matching_perm(self, db):
        userCreate = UserCreate(
            username=f"svc_rbac_nomatch_{uuid.uuid4().hex[:8]}",
            email=f"svc_rbac_nomatch_{uuid.uuid4().hex[:8]}@test.com",
            password="Test@123456",
        )
        user = UserService.create_user(db, userCreate)
        roleCreate = RoleCreate(
            name=f"svc_rbac_nomatch_role_{uuid.uuid4().hex[:8]}",
            desc="no match",
            permissions=["perm_a"],
        )
        role = RoleService.create_role(db, roleCreate)
        UserRoleService.assign_role(db, user.id, role.id)
        result = RBACService.check_permission(db, user.id, "perm_b")
        assert result is False
