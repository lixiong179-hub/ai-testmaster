...
@pytest.mark.asyncio
async def test_get_execution_history(db, eng_test_case, testUser):
    from app.models.test_result import TestResult
    from app.models.test_task import TestTask
    task = TestTask(
        task_name="history_test",
        project_id=eng_test_case.project_id,
        case_ids=[eng_test_case.id],
    ...
    for i in range(3):
        result = TestResult(
            task_id=task.id,
            project_id=eng_test_case.project_id,
            case_id=eng_test_case.id,
            case_no=eng_test_case.case_no,
            exec_status=1,
            exec_time=utcnow(),
        )
        db.add(result)
    ...
    history = await engine.get_execution_history(case_id=eng_test_case.id)
    assert len(history) == 3


@pytest.mark.asyncio
async def test_get_execution_history_with_limit(db, eng_test_case, testUser):
    from app.models.test_result import TestResult
    from app.models.test_task import TestTask
    task = TestTask(
        task_name="history_limit_test",
        project_id=eng_test_case.project_id,
        case_ids=[eng_test_case.id],
        executor_id=testUser.id,
        status=2,
    ...
    history = await engine.get_execution_history(case_id=eng_test_case.id, limit=2)
