...
class TestNormalizeOldFormat:
    ...
    def test_priority_unknown_value_fallback_P2(self):
        """未知priority值回退到P2"""
        case = {"priority": 99}
        result = _normalize_old_format(case)
        assert result["priority"] == "P2"
...
class TestTargetEnvWhitelist:
    ...
    def test_skip_init_true_accepted(self, client, auth_headers, execution_task):
        """skipInit=True 应被接受"""
        resp = client.post(f"/api/v1/testTask/{execution_task}/start",
                           json({"skipInit": True}),
                           headers=auth_headers)
    ...
    def test_combined_params_all_valid(self, client, auth_headers, execution_task):
        """组合参数：staging环境 + skipInit + headless=False"""
        resp = client.post(f"/api/v1/testTask/{execution_task}/start",
                           json({
                               "headless": False,
                               "recordVideo": True,
