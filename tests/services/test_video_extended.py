...
from app.services.video.legacy_models import VideoInfo as LegacyVideoInfo
...
from app.services.video.legacy_service import VideoService as LegacyVideoService, get_video_service
...
class TestLegacyVideoInfo:
    def test_to_dict(self):
        info = LegacyVideoInfo(
            id=1,
            task_id=100,
            case_id=200,
            file_path="/tmp/test.webm",
        ...
        d = info.to_dict()
        assert d["id"] == 1
        assert d["file_size_human"] == "1.00 KB"

    def test_format_file_size_bytes(self):
        assert LegacyVideoInfo._format_file_size(512) == "512.00 B"

    def test_format_file_size_kb(self):
        assert LegacyVideoInfo._format_file_size(2048) == "2.00 KB"

    def test_format_file_size_mb(self):
        assert LegacyVideoInfo._format_file_size(1048576) == "1.00 MB"

    def test_format_file_size_gb(self):
        assert LegacyVideoInfo._format_file_size(1073741824) == "1.00 GB"

    def test_format_file_size_tb(self):
        assert LegacyVideoInfo._format_file_size(1099511627776) == "1.00 TB"
...
class TestLegacyUtilsMixin:
    def test_get_video_path(self, tmp_path):
        mixin = LegacyUtilsMixin()
        mixin._video_base_dir = tmp_path
        path = mixin.get_video_path(1, 2)
        assert "task_1_case_2" in str(path)
        assert str(path).endswith(".webm")

    def test_get_video_url(self, tmp_path):
        mixin = LegacyUtilsMixin()
        mixin._video_base_dir = tmp_path
        video_path = str(tmp_path / "task_1_case_2_20260101.webm")
        url = mixin.get_video_url(video_path)
        assert url.startswith("/videos/")


class TestLegacyFfmpegMixin:
    def test_is_valid_video_path_outside_base(self, tmp_path):
        mixin = LegacyFfmpegMixin()
        mixin._video_base_dir = tmp_path
        assert mixin._is_valid_video_path("/other/path/video.mp4") is False

    def test_is_valid_video_path_within_base(self, tmp_path):
        mixin = LegacyFfmpegMixin()
        mixin._video_base_dir = tmp_path
        assert mixin._is_valid_video_path(str(tmp_path / "video.mp4")) is True

    @pytest.mark.asyncio
    async def test_check_ffmpeg_available_error(self):
        mixin = LegacyFfmpegMixin()
        with patch("asyncio.create_subprocess_exec", side_effect=Exception("error")):
            result = await mixin._check_ffmpeg_available()
            assert result is False

    @pytest.mark.asyncio
    async def test_check_ffprobe_available_error(self):
        mixin = LegacyFfmpegMixin()
        with patch("asyncio.create_subprocess_exec", side_effect=Exception("error")):
            result = await mixin._check_ffprobe_available()
            assert result is False

    @pytest.mark.asyncio
    async def test_extract_video_thumbnail_no_ffmpeg(self, tmp_path):
        mixin = LegacyFfmpegMixin()
        mixin._video_base_dir = tmp_path
        with patch.object(mixin, "_check_ffmpeg_available", return_value=False):
            result = await mixin.extract_video_thumbnail(str(tmp_path / "test.mp4"))
            assert result is None

    @pytest.mark.asyncio
    async def test_extract_video_thumbnail_invalid_path(self, tmp_path):
        mixin = LegacyFfmpegMixin()
        mixin._video_base_dir = tmp_path
        with patch.object(mixin, "_check_ffmpeg_available", return_value=True):
            result = await mixin.extract_video_thumbnail("/other/path/video.mp4")
            assert result is None

    @pytest.mark.asyncio
    async def test_get_video_duration_no_ffprobe(self, tmp_path):
        mixin = LegacyFfmpegMixin()
        mixin._video_base_dir = tmp_path
        with patch.object(mixin, "_check_ffprobe_available", return_value=False):
            result = await mixin.get_video_duration(str(tmp_path / "test.mp4"))
            assert result is None


class TestLegacyCleanupMixin:
    @pytest.mark.asyncio
    async def test_get_storage_stats_empty_dir(self, tmp_path):
        mixin = LegacyCleanupMixin()
        mixin._video_base_dir = tmp_path
        mixin._retention_days = 7
        mixin._max_storage_size_gb = 10
        result = await mixin.get_storage_stats()
    ...
    def test_set_retention_policy(self):
        ...
        mixin._retention_days = 7
        mixin._max_storage_size_gb = 10
        mixin.set_retention_policy(14, 20)
        assert mixin._retention_days == 14
        assert mixin._max_storage_size_gb == 20


class TestLegacyCrudMixin:
    @pytest.mark.asyncio
    async def test_get_video_info_not_found(self, db):
        mixin = LegacyCrudMixin()
        result = await mixin.get_video_info(db, 99999)
        assert result is None

    @pytest.mark.asyncio
    async def test_get_videos_by_task_empty(self, db):
        mixin = LegacyCrudMixin()
        result = await mixin.get_videos_by_task(db, 99999)
        assert result == []

    @pytest.mark.asyncio
    async def test_get_videos_by_case_empty(self, db):
        mixin = LegacyCrudMixin()
        result = await mixin.get_videos_by_case(db, 99999)
        assert result == []

    @pytest.mark.asyncio
    async def test_delete_video_not_found(self, db):
        mixin = LegacyCrudMixin()
        result = await mixin.delete_video(db, 99999)
        assert result is False
    ...
    async def test_save_and_get_video_info(self, db):
        mixin = LegacyCrudMixin()
        task_id, case_id = _create_parent_records(db)
        video_info = LegacyVideoInfo(
            id=0,
            task_id=task_id,
            case_id=case_id,
            file_path="/tmp/legacy_test.webm",
            file_size=2048,
        )
        record = await mixin.save_video_info(db, video_info)
        assert record.id is not None

        fetched = await mixin.get_video_info(db, record.id)
        assert fetched is not None
        assert fetched.file_path == "/tmp/legacy_test.webm"


class TestLegacyVideoService:
    def test_init(self, tmp_path):
        svc = LegacyVideoService(video_base_dir=str(tmp_path / "videos"))
        assert svc._retention_days == 7
        assert svc._max_storage_size_gb == 10

    def test_get_video_service_singleton(self, tmp_path):
        import app.services.video.legacy_service as mod
        original = mod._video_service
        mod._video_service = None
        try:
            svc1 = get_video_service()
            svc2 = get_video_service()
            assert svc1 is svc2
        finally:
            mod._video_service = original
