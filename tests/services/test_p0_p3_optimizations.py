...
class TestConfigSecurity:
    ...
    def test_empty_database_url_validation_exists(self):
        ...
        assert "未配�? in source or "禁止使用" in source or "sqlite" in source.lower(), \
            "Settings应有数据库URL校验"
    ...
    def test_prod_cors_validation_exists(self):
        ...
        assert ("prod" in source and ("通配�? in source or "wildcard" in source.lower())) or \
               "生产环境" in source, \
...
class TestTestCaseStepConversion:
    ...
    def test_convert_steps_basic_format(self):
        ...
        steps_json = [
            {"step": "打开浏览�?, "action": "click", "param": "登录按钮", "expected_result": "显示登录�?},
            {"step": "输入用户�?, "action": "type", "param": "admin", "expected_result": "输入框有�?}
        ]
        
        result = _convert_steps_to_response(steps_json)
        
        assert len(result) == 2
        assert result[0]["step"] == "打开浏览�?
        assert result[0]["action"] == "click"
        assert result[0]["param"] == "登录按钮"
        assert result[0]["expected_result"] == "显示登录�?
    ...
    def test_build_response_complete(self):
        ...
        assert result["precondition"] == "已注册账�?
...
class TestJWTUtilsUTCCompatibility:
    ...
    def test_utctnow_is_recent(self):
        ...
        assert diff < 5, f"_utcnow与实际时间差{diff}秒过�?
...
class TestTestCaseSoftDelete:
    ...
    def test_soft_delete_column_type(self):
        ...
        assert isinstance(columns.get("is_deleted"), type(Boolean())), \
            f"is_deleted应为Boolean类型，实�? {type(columns.get('is_deleted'))}"
        assert columns.get("deleted_at") is not None, "deleted_at字段应存�?
...
class TestBatchGenerateValidation:
    ...
    def test_description_min_length(self):
        ...
        with pytest.raises(Exception):  # ValidationError
            AIGenerateRequest(project_id=1, description="�?)
    ...
    def test_description_valid_length(self):
        ...
        req = AIGenerateRequest(project_id=1, description="这是一个有效的测试描述，长度足�?)
        
        assert req.description == "这是一个有效的测试描述，长度足�?
