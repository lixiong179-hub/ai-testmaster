"""冗余度算法升级 (T13) 单元测试 - 覆盖 TF-IDF 向量化、余弦相似度、动态阈值、RedundancyMixin。

测试策略: 真实数据构造，不使用 Mock，覆盖正常/空值/异常/边界场景。
"""
import math
import pytest
import numpy as np
...
class TestLightweightTfidfVectorizer:
    ...
    def test_l2_normalization(self) -> None:
        ...
        for i in range(matrix.shape[0]):
            ...
            if rowNorm > 0:
                assert abs(rowNorm - 1.0) < 1e-6, f"行 {i} 的 L2 范数不为 1: {rowNorm}"

    def test_identical_docs_high_similarity(self) -> None:
        """相同文档的 TF-IDF 向量应高度相似。"""
        docs = ["点击登录按钮 输入用户名", "点击登录按钮 输入用户名"]
        vectorizer = LightweightTfidfVectorizer()
        matrix = vectorizer.fit_transform(docs)
        sim = cosineSimilarity(matrix[0], matrix[1])
        assert sim > 0.99, f"相同文档相似度应接近 1.0，实际: {sim}"

    def test_different_docs_low_similarity(self) -> None:
        """差异大的文档相似度应较低。"""
        docs = ["点击登录按钮", "滚动页面到底部 等待加载完成"]
        vectorizer = LightweightTfidfVectorizer()
        matrix = vectorizer.fit_transform(docs)
        sim = cosineSimilarity(matrix[0], matrix[1])
        assert sim < 0.5, f"差异大的文档相似度应较低，实际: {sim}"

    def test_transform_after_fit(self) -> None:
        """transform 应使用已拟合的词汇表。"""
        trainDocs = ["点击登录 输入密码", "浏览商品 加入购物车"]
        vectorizer = LightweightTfidfVectorizer()
...
class TestBatchComputeSimilarity:
    ...
    def test_values_in_valid_range(self) -> None:
        """所有相似度值应在 [0, 1] 范围内。"""
        targets = ["点击登录 输入密码"]
        candidates = ["点击注册 输入邮箱", "浏览商品 加入购物车"]
        simMatrix = batchComputeSimilarity(targets, candidates)
        assert np.all(simMatrix >= 0.0)
        assert np.all(simMatrix <= 1.0)
...
class TestTfidfVsSequenceMatcher:
    ...
    def test_partial_overlap_not_false_positive(self) -> None:
        ...
        SequenceMatcher 对 "点击登录" vs "点击注册" 会给出较高相似度（共享"点击"），
        TF-IDF 应更精确地区分。
        """
        ...
        # TF-IDF 相似度应低于 SequenceMatcher，因为 TF-IDF 更关注区分性词汇
        assert tfidfSim < seqSim, (
            f"TF-IDF ({tfidfSim:.4f}) 应低于 SequenceMatcher ({seqSim:.4f}) "
            f"以减少误杀"
        )

    def test_truly_similar_docs_still_detected(self) -> None:
        """真正相似的文档仍应被检测出来。"""
        targets = ["点击登录按钮 输入用户名 输入密码 点击提交"]
        candidates = ["点击登录按钮 输入用户名 输入密码 点击提交 验证成功"]
        simMatrix = batchComputeSimilarity(targets, candidates)
        tfidfSim = float(simMatrix[0, 0])
        assert tfidfSim >= 0.7, f"真正相似的文档相似度应 >= 0.7，实际: {tfidfSim:.4f}"
    ...
    def test_completely_different_docs_low_similarity(self) -> None:
        ...
        simMatrix = batchComputeSimilarity(targets, candidates)
        tfidfSim = float(simMatrix[0, 0])
        assert tfidfSim < 0.3, f"完全不同的文档相似度应 < 0.3，实际: {tfidfSim:.4f}"
...
class TestRedundancyMixinLogic:
    ...
    def test_dynamic_threshold_integration(self) -> None:
        """动态阈值应与 _analyze_redundancy 集成正确。"""
        # 不同规模项目应产生不同阈值
