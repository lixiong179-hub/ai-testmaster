...
def mock_db_context(db):
    """Mock get_db_context to use in-memory SQLite session."""
    @contextmanager
    def _get_db_context():
        yield db
...
class TestQueryMetrics:
    def test_query_all(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", project_id=1)
            metrics_service.record_metric("low_confidence_pause", project_id=1)
            metrics_service.record_metric("json_validation_failure", project_id=1)

        result = metrics_service.query_metrics(db)
        assert len(result) == 2
    ...
    def test_query_by_metric_name(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", project_id=1)
            metrics_service.record_metric("json_validation_failure", project_id=1)

        result = metrics_service.query_metrics(db, metric_name="low_confidence_pause")
        assert len(result) == 1
        assert result[0]["metric_name"] == "low_confidence_pause"

    def test_query_by_project_id(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", project_id=1)
            metrics_service.record_metric("low_confidence_pause", project_id=2)

        result = metrics_service.query_metrics(db, project_id=1)
        assert len(result) == 1
        assert result[0]["count"] == 1

    def test_query_by_iteration_id(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", iteration_id=10)
            metrics_service.record_metric("low_confidence_pause", iteration_id=20)

        result = metrics_service.query_metrics(db, iteration_id=10)
        assert len(result) == 1

    def test_query_by_run_id(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", run_id=100)
            metrics_service.record_metric("low_confidence_pause", run_id=200)

        result = metrics_service.query_metrics(db, run_id=100)
        assert len(result) == 1

    def test_query_with_time_filter(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause")

        since = utcnow() - timedelta(days=1)
        result = metrics_service.query_metrics(db, since=since)
        assert len(result) >= 1

        future = utcnow() + timedelta(days=1)
        result = metrics_service.query_metrics(db, since=future)
        assert len(result) == 0
...
class TestGetMetricTimeseries:
    def test_timeseries_day(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", project_id=1)

        since = utcnow() - timedelta(days=7)
        result = metrics_service.get_metric_timeseries(
            db, "low_confidence_pause", project_id=1, since=since,
        )
        assert len(result) >= 1
        assert "time_bucket" in result[0]
        assert "count" in result[0]

    def test_timeseries_hour(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", project_id=1)

        since = utcnow() - timedelta(hours=24)
        result = metrics_service.get_metric_timeseries(
            db, "low_confidence_pause", project_id=1, since=since, interval="hour",
        )
        assert len(result) >= 1

    def test_timeseries_no_data(self, db):
        since = utcnow() - timedelta(days=7)
        result = metrics_service.get_metric_timeseries(
            db, "low_confidence_pause", since=since,
        )
        assert result == []


class TestGetDashboardSummary:
    def test_summary_with_data(self, db, mock_db_context):
        with patch("app.db.database.get_db_context", mock_db_context):
            metrics_service.record_metric("low_confidence_pause", project_id=1)
            metrics_service.record_metric("json_validation_failure", project_id=1)

        since = utcnow() - timedelta(days=30)
        result = metrics_service.get_dashboard_summary(db, project_id=1, since=since)
