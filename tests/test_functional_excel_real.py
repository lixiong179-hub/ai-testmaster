...
class TestFunctionalExcelReal(unittest.TestCase):
    ...
    def test_07_export_to_functional_excel_success(self):
        ...
        self.assertEqual(headers, ["用例序号", "优先�?, "用例描述", "初始条件", "操作步骤", "期望结果", "测试结果", "测试�?])
    ...
    def test_10_build_functional_expected(self):
        ...
        steps = [
            ...
            TestStep(step_number=3, action="步骤3", expected_result="")  # 空预�?
        ]
        ...
        self.assertNotIn('[3]', result)  # 空预期不应包�?
    ...
    def test_14_import_export_round_trip(self):
        ...
        self.assertEqual(ws.cell(row=2, column=1).value, original_data[0]['所属模�?])
        self.assertEqual(ws.cell(row=3, column=2).value, 'P2')
        print("�?测试14通过: 导入导出往返测�?)
