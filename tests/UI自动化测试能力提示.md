...
class VisionClient:
    ...
    def __init__(self, api_key: str, redis_client):
        ...
        self.base_url = "https://api.moonshot.cn/v1"  # 实际地址以官方为准
...
使用Kimi对比公版和定制版的截图，输出变更列表（元素新增/移除/移动），并评估对测试的影响。
...
- **Kimi K2.5多模态**：按输入图像+文本 tokens 计费，约¥0.02/千tokens（假设）。
- **DeepSeek**：¥0.001/千tokens（输入），¥0.002/千tokens（输出）。
...
- 跳转验证（每个页面5个跳转，首次验证）：5 \* 100 \* 0.02 = 100元。
- 自然语言测试生成（每个任务1次DeepSeek）：100 \* 0.001 ≈ 0.1元。
- 报告分析（每个任务1次DeepSeek）：100 \* 0.002 ≈ 0.2元。
- 总计约700元/月，若缓存命中率90%，可降至70元/月左右。
