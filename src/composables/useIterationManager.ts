...
export function useIterationManager() {
  ...
  const validIterationsForSelect = computed<SafeIteration[]>(() =>
    safeIterations.value.filter((it) => it.id)
  )
  ...
  const handleDeleteIteration = async (iteration: Iteration): Promise<boolean> => {
    try {
      ...
      if (error !== 'cancel') formManager.showError('删除迭代', error)
      return false
    }
  }
  ...
  const handleIterationCommand = async (
    ...
    if (command === 'edit') {
      formManager.handleEditIteration(iteration)
      return false
    } else if (command === 'delete') return await handleDeleteIteration(iteration)
    return false
  }

  const getIterationStatusType = (status: string): TagType => {
  ...
  const getIterationStatsById = (id: number): { files: number; prototypes: number } => {
    ...
    return { files: stats?.files ?? 0, prototypes: stats?.prototypes ?? 0 }
  }
  ...
  return reactive({
    ...
    get isSubmitting() {
      return formManager.submitting.value === true
    },
    loadIterations,
    ...
    showError: formManager.showError,
  })
}
