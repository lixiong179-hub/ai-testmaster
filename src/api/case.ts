...
export const testCaseApi = {
  ...
  updateStepLocator: async (caseId: number, stepId: number, data: StepLocatorUpdateData): Promise<Record<string, unknown>> => {
    const response = await request.put(`/api/v1/testCase/${caseId}/steps/${stepId}/locator`, data);
    return extractResponseData<Record<string, unknown>>(response as unknown as ApiResponse<Record<string, unknown>> | Record<string, unknown>);
  },

  getCaseVersions: async (caseId: number, page: number = 1, pageSize: number = 20): Promise<CaseVersionPageResponse> => {
    const response = await request.get(`/api/v1/testCase/${caseId}/versions`, { params: { page, page_size: pageSize } });
    return extractResponseData<CaseVersionPageResponse>(response as unknown as ApiResponse<CaseVersionPageResponse> | CaseVersionPageResponse);
  },
  ...
  getCaseVersionDetail: async (caseId: number, versionId: number): Promise<CaseVersionDetail> => {
    ...
    return extractResponseData<CaseVersionDetail>(response as unknown as ApiResponse<CaseVersionDetail> | CaseVersionDetail);
  },

  compareCaseVersions: async (caseId: number, fromVersion: number, toVersion: number): Promise<CaseVersionCompareResult> => {
    const response = await request.get(`/api/v1/testCase/${caseId}/versions/compare`, { params: { from: fromVersion, to: toVersion } });
    return extractResponseData<CaseVersionCompareResult>(response as unknown as ApiResponse<CaseVersionCompareResult> | CaseVersionCompareResult);
  },
  ...
  startCorrection: async (caseId: number): Promise<CorrectionResponse> => {
    ...
    return extractResponseData<CorrectionResponse>(response as unknown as ApiResponse<CorrectionResponse> | CorrectionResponse);
  },
  ...
  submitVerification: async (caseId: number): Promise<VerificationResponse> => {
    ...
    return extractResponseData<VerificationResponse>(response as unknown as ApiResponse<VerificationResponse> | VerificationResponse);
  },
  ...
  batchDeleteCases: async (caseIds: number[]): Promise<{ success_count: number; fail_count: number; not_found_count: number; deleted_ids: number[]; total: number; message: string }> => {
    const response = await request.post('/api/v1/testCase/batch-delete', { caseIds: caseIds });
    return extractResponseData<{ success_count: number; fail_count: number; not_found_count: number; deleted_ids: number[]; total: number; message: string }>(response as unknown as ApiResponse<{ success_count: number; fail_count: number; not_found_count: number; deleted_ids: number[]; total: number; message: string }> | { success_count: number; fail_count: number; not_found_count: number; deleted_ids: number[]; total: number; message: string });
  },

  batchRestoreCases: async (caseIds: number[]): Promise<{ success_count: number; fail_count: number; not_found_count: number; restored_ids: number[]; total: number; message: string }> => {
    const response = await request.post('/api/v1/testCase/batch-restore', { caseIds: caseIds });
    return extractResponseData<{ success_count: number; fail_count: number; not_found_count: number; restored_ids: number[]; total: number; message: string }>(response as unknown as ApiResponse<{ success_count: number; fail_count: number; not_found_count: number; restored_ids: number[]; total: number; message: string }> | { success_count: number; fail_count: number; not_found_count: number; restored_ids: number[]; total: number; message: string });
  },
  ...
  aiGenerateCaseEnhanced: async (data: TestCaseAIEnhancedRequest): Promise<AIEnhancedGenerateResponse> => {
    const response = await request.post('/api/v1/testCase/ai-generate-enhanced', data);
    return extractResponseData<AIEnhancedGenerateResponse>(response as unknown as ApiResponse<AIEnhancedGenerateResponse> | AIEnhancedGenerateResponse);
  },

  generate: async (data: TestCaseGenerateRequest): Promise<AsyncGenerator<{ progress: number; message?: string }>> => {
  ...
  retry: async (projectId: number, data: TestCaseRetryRequest): Promise<AsyncGenerator<{ progress: number; message?: string }>> => {
  ...
  importCases: async (file: File): Promise<ImportResult> => {
    ...
    return extractResponseData<ImportResult>(response as unknown as ApiResponse<ImportResult> | ImportResult);
  }
};
