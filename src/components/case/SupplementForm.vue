    ...
    <el-tabs v-model="activeTab">
        ...
        <div v-if="hasInferredCapabilities" class="capability-section">
          <div class="section-toolbar">
            <span class="toolbar-stat"
              >共 {{ inferredCapabilities.length }} 项，已选 {{ selectedInferredCount }} 项</span
            >
            <div class="toolbar-actions">
              <el-button link type="primary" size="small" @click="selectAllInferred"
                >全选</el-button
              >
              <el-button link type="danger" size="small" @click="deselectAllInferred"
                >清空</el-button
              >
            </div>
          </div>
          <div class="capability-list">
            <div
              v-for="(cap, index) in inferredCapabilities"
              :key="index"
              class="capability-item"
              :class="{ selected: cap.selected }"
              @click="toggleInferredCapability(index)"
            >
              <el-checkbox
                :model-value="cap.selected"
                @change="toggleInferredCapability(index)"
                @click.stop
              />
      ...
      <el-tab-pane label="自定义能力" name="custom">
        <div class="custom-capability-section">
          <div
            v-for="(cap, index) in customCapabilities"
            :key="index"
            class="custom-capability-item"
          >
            ...
            <el-button
              type="danger"
              link
              @click="removeCustomCapability(index)"
              style="margin-left: 8px"
              ><el-icon><Delete /></el-icon
            ></el-button>
          </div>
          <el-button type="primary" plain @click="addCustomCapability" style="margin-top: 8px"
            ><el-icon><Plus /></el-icon>添加自定义能力</el-button
          >
        </div>
      </el-tab-pane>
...
const _emit = defineEmits<{ cancel: []; saved: [] }>()
