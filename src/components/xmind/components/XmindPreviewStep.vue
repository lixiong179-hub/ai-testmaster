  ...
  <div v-if="currentStep === 1" class="preview-area">
    ...
    <div class="preview-header">
      ...
      <el-tag v-if="previewCaseData.length > 0" type="primary"
        >测试用例 {{ previewCaseData.length }} 条</el-tag
      >
      ...
      <el-tag v-if="previewSkippedCount > 0" type="warning"
        >已跳过 {{ previewSkippedCount }} 个节点</el-tag
      >
    </div>
    <el-alert
    ...
    >
      <template #title
        >检测到
        {{ previewSkippedCount }} 个节点未导入预览，常见原因是模块名或测试点名称为空。</template
      >
    ...
    >
      <template #title
        >该 XMind 将按"测试点 + 测试用例"双通道导入，可切换查看两种预览视图。</template
      >
    </el-alert>
    <el-alert
    ...
    >
      <template #title
        >AI增强解析超时，已自动降级为普通解析模式。如需使用AI增强，请稍后重试或检查网络连接。</template
      >
    </el-alert>
    <el-alert
    ...
    >
      <template #title
        ...
        {{ previewTotalPaths }} 条）。确认导入时将对全部路径进行 AI 解析，预计耗时较长。</template
      >
    </el-alert>
