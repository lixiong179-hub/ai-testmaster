<template>
        ...
        <template #title>
          <span class="collapse-title">
            ...
            <el-tag size="small" type="info" style="margin-left: 8px"
              >{{ ctx.taskResults.value.length }} 条</el-tag
            >
          </span>
        </template>
        <el-table
          v-loading="ctx.loading.value"
          :data="ctx.taskResults.value"
          style="width: 100%"
          border
          stripe
          max-height="400"
        >
          ...
          <el-table-column prop="exec_status" label="执行状态" width="120" align="center">
            <template #default="scope">
              <el-tag :type="ctx.execStatusColor(scope.row.exec_status)" effect="dark">{{
                ctx.execStatusText(scope.row.exec_status)
              }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="280" align="center">
            <template #default="scope">
              <el-space>
                <el-button type="info" size="small" link @click="ctx.viewCaseLog(scope.row)"
                  >查看日志</el-button
                >
                <el-button
                  ...
                  size="small"
                  link
                  @click="ctx.viewScreenshot(scope.row)"
                  >查看截图</el-button
                >
                <el-button
      ...
      <el-collapse-item title="执行日志" name="logs">
        <template #title>
          <span class="collapse-title">
            ...
            <el-badge
              v-if="ctx.executionLogs.value.length > 0"
              :value="ctx.executionLogs.value.length"
              type="primary"
        ...
        <div class="log-toolbar">
          <el-button size="small" @click="ctx.clearLogs"
            ><el-icon><Delete /></el-icon>清空日志</el-button
          >
          <el-button
            size="small"
            @click="ctx.exportLogs"
            :disabled="ctx.executionLogs.value.length === 0"
            ><el-icon><Download /></el-icon>导出日志</el-button
          >
          <el-checkbox v-model="ctx.autoScroll.value" size="small" style="margin-left: 10px"
            >自动滚动</el-checkbox
          >
        </div>
        <el-scrollbar style="height: 400px" :ref="setScrollbarRef">
          <div class="log-container" :ref="setLogContainerRef">
            <div
              v-for="(item, index) in ctx.executionLogs.value"
              :key="index"
              :class="['log-item', ctx.getLogItemClass(item.status)]"
            >
              <div class="log-header">
                <span class="log-time">{{ ctx.formatTime(item.timestamp) }}</span>
                <span class="log-case" v-if="item.case_no">[{{ item.case_no }}]</span>
                <el-tag :type="ctx.logStatusColor(item.status)" size="small">{{
                  ctx.logStatusText(item.status)
                }}</el-tag>
              </div>
  ...
  <el-dialog v-model="ctx.logDialogVisible.value" title="用例执行日志" width="80%" top="5vh">
    <div class="case-log">
      <pre>{{ ctx.currentCaseLog.value }}</pre>
    </div>
    <template #footer>
  ...
  <el-dialog v-model="ctx.screenshotDialogVisible.value" title="失败截图" width="80%" top="5vh">
    <div class="screenshot-preview">
      <el-image
        v-if="ctx.currentScreenshot.value"
        :src="ctx.currentScreenshot.value"
        :preview-src-list="[ctx.currentScreenshot.value]"
        fit="contain"
  ...
  <el-dialog v-model="ctx.errorDialogVisible.value" title="错误详情" width="60%" top="10vh">
    <div class="error-detail">
      <pre>{{ ctx.currentError.value }}</pre>
    </div>
  </el-dialog>
</template>
...
const setLogContainerRef = (el: unknown) => {
  ctx.logContainerRef.value = el instanceof HTMLElement ? el : null
}
</script>
