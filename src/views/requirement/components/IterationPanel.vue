  ...
  <div class="iteration-panel">
    <div class="iteration-panel-header">
      ...
      <el-button
        type="primary"
        size="small"
        @click="ctx.handleAddIterationWrapper"
    ...
    <div class="iteration-list" v-if="ctx.resourceList.filterForm.project_id">
      <div
        ...
        :class="{ active: ctx.iterationManager.selectedIterationId === null }"
        @click="ctx.handleSelectIterationAndRefresh(null)"
      >
        <div class="iteration-card-info"><span class="iteration-name">全部</span></div>
      </div>

      <template
        v-for="(iteration, index) in ctx.safeIterationsArray.value"
        :key="'panel-' + (iteration?.id ?? index)"
      >
        <div
          v-if="iteration && iteration.id"
          class="iteration-card"
          :class="{ active: ctx.iterationManager.selectedIterationId === iteration.id }"
          @click="ctx.handleSelectIterationAndRefresh(iteration.id)"
        >
          <div class="iteration-card-info">
            ...
            <div class="iteration-stats" v-if="iteration && iteration.id">
              <span
                class="stat-item"
                v-if="
                  ctx.getIterationStats(iteration.id).files > 0 ||
                  ctx.getIterationStats(iteration.id).prototypes > 0
                "
              >
                <el-icon><Document /></el-icon
                >{{ ctx.getIterationStats(iteration.id).files }} 个文档
              </span>
              <span class="stat-item" v-if="ctx.getIterationStats(iteration.id).prototypes > 0">
                <el-icon><Picture /></el-icon
                >{{ ctx.getIterationStats(iteration.id).prototypes }} 个原型
              </span>
            </div>
            <div class="iteration-meta">
              <el-tag
                :type="ctx.iterationManager.getIterationStatusType(iteration?.status)"
                size="small"
              >
              ...
              <span @click.stop>
                <el-dropdown
                  trigger="click"
                  @command="
                    (cmd: string) => iteration && ctx.handleIterationCommandWrapper(cmd, iteration)
                  "
                >
      ...
      <div
        class="iteration-card"
        :class="{ active: ctx.iterationManager.selectedIterationId === 0 }"
        @click="ctx.handleSelectIterationAndRefresh(0)"
      >
        <div class="iteration-card-info">
          <div class="iteration-name-row">
    ...
    <el-dialog
      v-model="ctx.iterationManager.iterationDialogVisible"
      :title="ctx.iterationManager.iterationDialogMode === 'add' ? '新建迭代' : '编辑迭代'"
      width="520px"
      :close-on-click-modal="false"
    >
      <el-form
        :ref="setIterationFormRef"
        :model="ctx.iterationManager.iterationFormData"
        :rules="ctx.iterationManager.iterationFormRules"
        label-width="100px"
      >
        <el-form-item label="迭代名称" prop="name"
          ><el-input
            v-model="ctx.iterationManager.iterationFormData.name"
            placeholder="请输入迭代名称"
        /></el-form-item>
        <el-form-item label="版本号" prop="version"
          ><el-input v-model="ctx.iterationManager.iterationFormData.version" placeholder="如 v1.0"
        /></el-form-item>
        <el-form-item label="描述"
          ><el-input
            v-model="ctx.iterationManager.iterationFormData.description"
            type="textarea"
            :rows="2"
            placeholder="迭代描述（可选）"
        /></el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select
            v-model="ctx.iterationManager.iterationFormData.status"
            placeholder="请选择状态"
            style="width: 100%"
          >
            <el-option
        ...
        <el-form-item label="开始日期"
          ><el-date-picker
            v-model="ctx.iterationManager.iterationFormData.start_date"
            type="date"
            placeholder="选择开始日期"
            style="width: 100%"
        ...
        <el-form-item label="结束日期"
          ><el-date-picker
            v-model="ctx.iterationManager.iterationFormData.end_date"
            type="date"
            placeholder="选择结束日期"
            style="width: 100%"
            value-format="YYYY-MM-DD"
      ...
      <template #footer>
        ...
        <el-button
          type="primary"
          @click="ctx.handleIterationSubmitWrapper"
          :loading="ctx.iterationManager.isSubmitting"
          >确定</el-button
        >
      </template>
    </el-dialog>
...
const setIterationFormRef = (el: unknown) => {
  ctx.iterationFormLocalRef.value = el
}
</script>
