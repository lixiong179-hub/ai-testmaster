...
export function useProjectList() {
  ...
  const filteredProjects = computed(() => {
    ...
    return projectStore.projects.filter((p) => p.name.toLowerCase().includes(searchKeyword.value.toLowerCase()))
  })

  const getStatusType = (status: number) => {
    const m: Record<number, string> = { 0: 'info', 1: 'success', 2: 'warning' }
  ...
  const handleSearch = () => { projectStore.fetchProjects() }

  const handleCurrentChange = (current: number) => {
    projectStore.currentPage = current; projectStore.fetchProjects()
  }

  const handleSizeChange = (size: number) => {
    projectStore.pageSize = size; projectStore.currentPage = 1; projectStore.fetchProjects()
  }

  const openCreateDialog = () => {
    projectForm.value = {
      name: '', description: '', project_type: 'web' as 'web' | 'app',
      web_env_configs: {
        test: { url: '', username: '', password: '' },
        staging: { url: '', username: '', password: '' },
        prod: { url: '', username: '', password: '' },
  ...
  const createProject = async () => {
    ...
    await projectFormRef.value.validate(async (valid: boolean) => {
      if (valid) {
        ...
        if (projectId) { dialogVisible.value = false; goToDetail(projectId) }
      }
    })
  }

  const goToDetail = (projectId: number) => { router.push(`/home/project/detail?id=${projectId}`) }
  const goToTaskList = (projectId: number) => { router.push(`/home/task/list/${projectId}`) }
  const goToTestPointManagement = (projectId: number) => {
    router.push({ path: '/home/case/test-point-management', query: { projectId: String(projectId) } })
  }

  const confirmDelete = (projectId: number) => {
    ElMessageBox.confirm('确定要删除此项目吗？删除后将级联删除关联的文件、测试点和用例。', '提示', {
      confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning',
    }).then(async () => { await projectStore.deleteProject(projectId) }).catch(() => {})
  }
