...
export function useProjectDetail() {
  ...
  const loadConfig = () => {
    const project = projectStore.currentProject as (Project & { web_env_configs?: WebEnvConfigs; device_config?: DeviceConfig }) | null
    if (!project) return
    if (project.project_type === 'web' && project.web_env_configs) {
      const cfg = project.web_env_configs
      if (cfg.test) webEnvForm.test = { ...webEnvForm.test, ...cfg.test, password: cfg.test.password === PASSWORD_MASK ? '' : cfg.test.password || '' }
      if (cfg.staging) webEnvForm.staging = { ...webEnvForm.staging, ...cfg.staging, password: cfg.staging.password === PASSWORD_MASK ? '' : cfg.staging.password || '' }
      if (cfg.prod) webEnvForm.prod = { ...webEnvForm.prod, ...cfg.prod, password: cfg.prod.password === PASSWORD_MASK ? '' : cfg.prod.password || '' }
  ...
  const saveWebEnvConfig = async () => {
    ...
    try {
      const buildPayload = (envData: { url: string; username: string; password: string }) => {
        ...
        if (envData.password && envData.password !== PASSWORD_MASK) payload.password = envData.password
        return payload
      }
      const res = await ProjectAPI.updateProjectConfig(projectId.value, {
        project_type: 'web',
        web_env_configs: { test: buildPayload(webEnvForm.test), staging: buildPayload(webEnvForm.staging), prod: buildPayload(webEnvForm.prod) } as WebEnvConfigs,
      })
  ...
  const saveDeviceConfig = async () => {
    ...
    try {
      const res = await ProjectAPI.updateProjectConfig(projectId.value, {
        ...
        device_config: {
          default_device: {
            platform: deviceForm.platform || undefined, device_id: deviceForm.device_id || undefined,
            device_name: deviceForm.device_name || undefined, app_package: deviceForm.app_package || undefined,
  ...
  const handleUploadSuccess = async () => { await projectStore.fetchProjectDetail(projectId.value) }

  const confirmDeleteFile = (fileId: number) => {
    ElMessageBox.confirm('确定要删除此文件吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
      .then(async () => { await projectStore.deleteFile(fileId, projectId.value) })
      .catch(() => {})
  }

  const goBack = () => { router.push('/home/project') }
  ...
  onMounted(async () => {
    ...
    if (id) {
      ...
      ElMessage.error('项目ID不能为空'); goBack()
    }
  })

  onUnmounted(() => { projectStore.resetState() })
