...
export function useReviewInbox() {
  ...
  const filteredDecisions = computed(() => {
    ...
    if (
      ...
      activeTab.value === 'deprecate'
    )
      result = result.filter((d) => d.ai_verdict === activeTab.value)
  ...
  const statusTagType = computed<TagType>(() => {
    ...
    return m[reviewStatus.value] || 'info'
  })
  const statusText = computed(() => {
    ...
    return m[reviewStatus.value] || reviewStatus.value
  })
  const hasHighConfidenceUndecided = computed(() =>
    decisions.value.some((d) => d.human_verdict === null && (d.ai_confidence ?? 0) >= 85)
  )
  ...
  function verdictLabel(verdict?: string): string {
    const m: Record<string, string> = { keep: '采纳', modify: '需修改', deprecate: '废弃' }
    return m[verdict || ''] || verdict || '—'
  }
  function verdictTagType(verdict?: string): TagType {
    const m: Record<string, TagType> = { keep: 'success', modify: 'warning', deprecate: 'danger' }
    return m[verdict || ''] || 'info'
  }
  function confidenceColor(value: number): string {
    return value >= 85 ? '#67c23a' : value >= 60 ? '#e6a23c' : '#f56c6c'
  }
  function isSelectable(): boolean {
    return reviewStatus.value !== 'finalized'
  }
  function toggleHint(row: ReviewDecision) {
    expandedHints.has(row.id) ? expandedHints.delete(row.id) : expandedHints.add(row.id)
  }
  ...
  async function handleDecide(row: ReviewDecision, verdict: string) {
    try {
      await reviewApi.decideSingle(reviewId.value, row.id, { human_verdict: verdict })
      ElMessage.success('判定已提交')
      await fetchDecisions()
    } catch (e: unknown) {
      const err = e as { response?: { status: number; data?: { msg?: string } }; message?: string }
      if (err?.response?.status === 409) ElMessage.warning('该决策目标已被其他用户锁定，请稍后重试')
      else if (err?.response?.status === 400)
        ElMessage.warning(err?.response?.data?.msg || '操作失败：评审已最终化或状态异常')
      else ElMessage.error(err?.response?.data?.msg || err?.message || '提交判定失败')
    }
  }
    ...
    ElMessageBox.confirm(
      ...
      .then(async () => {
        try {
          ...
          if (err?.response?.status === 403) ElMessage.warning('撤销窗口已过期（超 1h），无法撤销')
          else ElMessage.error(err?.response?.data?.msg || '撤销失败')
        }
      })
      .catch(() => {})
  }
  ...
  function handleBatchAcceptHighConfidence() {
    const hc = decisions.value.filter(
      (d) => d.human_verdict === null && (d.ai_confidence ?? 0) >= 85
    )
    if (!hc.length) {
      ElMessage.info('没有可批量采纳的高置信度决策')
      return
    }
    ElMessageBox.confirm(`将采纳 ${hc.length} 条高置信度决策，是否继续？`, '批量采纳', {
      type: 'warning',
    })
      .then(async () => {
        try {
          await reviewApi.decideBatch(reviewId.value, {
            decisions: hc.map((d) => ({ decision_id: d.id, human_verdict: 'keep' as string })),
          })
          ElMessage.success(`已批量采纳 ${hc.length} 条决策`)
          await fetchDecisions()
        } catch (e: unknown) {
          const err = e as {
            response?: { status: number; data?: { msg?: string } }
          ...
          if (err?.response?.status === 409) ElMessage.warning('部分决策目标已被锁定，批量采纳失败')
          else ElMessage.error(err?.response?.data?.msg || err?.message || '批量采纳失败')
        }
      })
      .catch(() => {})
  }
  ...
  function handleFinalize() {
    ElMessageBox.confirm('最终化后所有决策将不可修改，是否确认？', '最终化评审', {
      type: 'warning',
      ...
      .then(async () => {
        ...
        try {
          ...
          else ElMessage.error(err?.response?.data?.msg || '最终化失败')
        } finally {
