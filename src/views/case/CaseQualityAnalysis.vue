    ...
    <div v-if="analysisResult" class="analysis-content" v-loading="loading">
      ...
      <div class="analysis-cards">
        <el-card
          ...
          class="dim-card"
          shadow="hover"
          @click="showDetail(dim.detail || dim.name)"
        >
          <div class="dim-header">
            <span class="dim-name">{{ dim.label || dim.name }}</span
            ><el-tag
              :type="dim.score >= 80 ? 'success' : dim.score >= 60 ? 'warning' : 'danger'"
              size="small"
              >{{ dim.score }}分</el-tag
            >
          </div>
          ...
          <div v-if="dim.suggestions && dim.suggestions.length" class="dim-suggestions">
            <div v-for="(s, i) in dim.suggestions.slice(0, 3)" :key="i" class="suggestion-item">
              <el-icon color="#e6a23c"><Warning /></el-icon><span>{{ s }}</span>
            </div>
          </div>
        </el-card>
      </div>
      ...
      <div v-if="costSummary" class="cost-section" v-loading="costLoading">
        <h3>成本统计</h3>
        <el-descriptions :column="3" border size="small">
          <el-descriptions-item label="总Token消耗">{{
            costSummary.total_tokens ?? '-'
          }}</el-descriptions-item>
          <el-descriptions-item label="总API调用次数">{{
            costSummary.total_api_calls ?? '-'
          }}</el-descriptions-item>
          <el-descriptions-item label="估算成本">{{
            costSummary.estimated_cost ?? '-'
          }}</el-descriptions-item>
        </el-descriptions>
      </div>
    </div>
