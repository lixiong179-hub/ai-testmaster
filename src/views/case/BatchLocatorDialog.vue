<template>
  <el-dialog v-model="visible" title="批量元素定位" width="700px" :close-on-click-modal="false" @close="close">
    <div v-if="currentView === 'config'" class="config-section">
      ...
      <el-form label-width="100px">
        ...
        <el-form-item label="AI辅助">
          ...
          <div style="margin-top: 4px; font-size: 12px; color: #909399">启用后AI将根据页面截图和DOM结构智能推荐定位器</div>
        </el-form-item>
      </el-form>
    </div>
    ...
    <div v-else class="report-section">
      ...
      <div class="report-summary" v-if="hasResults">
        <el-descriptions :column="3" border size="small">
          ...
          <el-descriptions-item label="成功"><el-tag type="success">{{ successCount }}</el-tag></el-descriptions-item>
          <el-descriptions-item label="失败"><el-tag type="danger">{{ failCount }}</el-tag></el-descriptions-item>
        </el-descriptions>
      </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button v-if="currentView === 'config'" type="primary" :loading="loading" @click="startLocate">开始定位</el-button>
        <el-button v-if="currentView === 'report' && failCount > 0" type="warning" @click="retryFailed">重试失败项</el-button>
        <el-button v-if="currentView === 'report' && successCount > 0" type="primary" @click="applyResults">应用结果</el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
.progress-section { padding: 20px 0; text-align: center; }
.progress-message { margin: 16px 0; color: #606266; }
.progress-stats { display: flex; justify-content: center; gap: 12px; margin-top: 16px; }
