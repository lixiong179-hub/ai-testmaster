  ...
  <div class="case-detail">
    <el-card>
      <template #header>
        <div class="page-header">
          <div class="header-left">
            <el-button type="info" @click="goBack" :icon="ArrowLeft">
              返回
            </el-button>
            <h2>测试用例详情</h2>
            ...
            <el-tag v-if="isCorrectionMode" type="warning" effect="dark">
              纠正模式
            </el-tag>
          </div>
          <div class="header-right">
            ...
            <el-button type="success" @click="copyCase" :icon="DocumentCopy" :disabled="issueType === 'product_bug'">
              复制用例
            </el-button>
            <el-button @click="openVersionHistory" :disabled="!caseId">
              版本历史
            </el-button>
            <el-button
      ...
      <div v-else-if="caseItem" class="case-content">
        ...
        <!-- 快速验证按钮 -->
        <div v-if="isCorrectionMode && issueType !== 'product_bug' && currentView === VIEW_TYPES.TECHNICAL" class="quick-verify-bar">
          <el-button type="success" @click="openQuickVerify" :icon="Check">
            快速验证
          </el-button>
        ...
        <div class="section">
          ...
          <div v-else-if="currentView === VIEW_TYPES.TECHNICAL" class="steps-view">
            ...
            <div v-else-if="technicalViewData">
              ...
              <!-- 前置条件步骤 -->
              <div v-if="technicalViewData.precondition" class="precondition-section" style="margin-top: 16px;">
                <div class="precondition-header" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
                  <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <el-tag v-if="technicalViewData.precondition_steps?.length" size="small" type="info">
                ...
                >
                  ...
                  <el-table-column label="操作类型" width="90" align="center">
                    <template #default="{ row }">
                      <el-tag size="small" :type="getActionTypeTagType(row.action_type)">{{ row.action_type || '-' }}</el-tag>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column label="定位类型" width="110" align="center">
                    <template #default="{ row }">
                      <el-tag v-if="row.locator?.locator_type" size="small"
                  ...
                  <el-table-column label="定位值" min-width="200">
                    <template #default="{ row }">
                      <span v-if="row.locator?.locator_value" style="font-size: 12px; word-break: break-all;">
                  ...
                  <el-table-column label="操作" width="120" align="center">
                    <template #default="{ $index }">
                      <el-button size="small" type="danger" text @click="deletePreconditionStep($index)">删除</el-button>
                    </template>
                  </el-table-column>
                </el-table>
              ...
              <!-- 智能建议面板 -->
              <div v-if="isCorrectionMode && (failureReason || aiAnalysisText)" class="suggestion-panel">
              ...
              >
                  ...
                  <template #default="{ row, $index }">
                    ...
                    >
                      ...
                      <div class="cell-actions">
                        <el-button size="small" type="success" @click="saveCellEdit" :loading="cellSaving">保存</el-button>
                        <el-button size="small" @click="cancelCellEdit">取消</el-button>
                      </div>
                    </div>
                    <div v-else class="cell-display" @dblclick="startCellEdit($index, 'action', row.action)">
                      <span>{{ row.action || '-' }}</span>
                      <el-icon v-if="issueType !== 'product_bug'" class="edit-icon"><Edit /></el-icon>
                    </div>
                  </template>
                </el-table-column>
                <el-table-column label="预期结果" min-width="200">
                  <template #default="{ row, $index }">
                    <div
                      v-if="editingCell?.stepIndex === $index && editingCell?.field === 'expected_result'"
                      class="cell-editing"
                    >
                      <el-input v-model="editingValue" size="small" />
                      <div class="cell-actions">
                    ...
                    <div v-else class="cell-display" @dblclick="startCellEdit($index, 'expected_result', row.expected_result)">
                ...
                <el-table-column label="定位值" min-width="200">
                  <template #default="{ row }">
                    <span>{{ row.locator?.locator_value || row.locator?.css_selector || '-' }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="XPath" min-width="200">
                  <template #default="{ row, $index }">
                    ...
                    <div v-else class="cell-display" @dblclick="startCellEdit($index, 'xpath', row.locator?.xpath)">
                ...
                <el-table-column label="操作" width="100" align="center" v-if="issueType !== 'product_bug'">
                  <template #default="{ row, $index }">
                    <el-button
        ...
        <div v-if="isEditing" class="save-actions">
          ...
          <el-button size="large" @click="cancelEdit" :icon="Close">
            取消
          </el-button>
        </div>
      </div>
    ...
    <el-dialog v-model="quickVerifyVisible" title="快速验证" width="500px">
      ...
      <el-checkbox-group v-model="selectedVerifySteps">
        <div v-if="technicalViewData" class="verify-step-list">
          ...
          >
            步骤 {{ step.step_number }}: {{ step.action?.substring(0, 50) || '-' }}{{ step.action?.length > 50 ? '...' : '' }}
          </el-checkbox>
        </div>
      </el-checkbox-group>
      <template #footer>
      ...
      <div v-else>
        <el-table :data="versionList" style="width: 100%;" border stripe>
          ...
          <el-table-column label="修改类型" width="100" align="center">
            <template #default="{ row }">
              <el-tag
                :type="row.change_type === 'correction' ? 'warning' : row.change_type === 'rollback' ? 'info' : ''"
                size="small"
              >
    ...
    <el-dialog v-model="addLocatorVisible" title="添加元素定位" width="500px">
      <el-form :model="addLocatorForm" label-width="100px">
        <el-form-item label="CSS选择器">
          <el-input v-model="addLocatorForm.css_selector" placeholder="例如: #username, .login-btn" />
        </el-form-item>
...
const copyCase = async () => {
  ...
  const caseText = `用例编号: ${caseItem.value.case_no}\n` +
  ...
  try {
    ...
    if (field === 'action' || field === 'expected_result') {
      const stepsPayload = technicalViewData.value.steps.map((s, i) => ({
        ...
        expected_result: i === stepIndex && field === 'expected_result' ? editingValue.value : (s.expected_result || '')
      }));
