...
export function useBatchLocator(projectId: number) {
  ...
  async function cancelTask(): Promise<void> {
    ...
    try {
      await cancelBatchRecord(currentTaskId.value)
  ...
  /** 加载批量定位任务列表 */
  async function loadBatchTasks(): Promise<void> {
    tasksLoading.value = true
    try {
      const tasks = await listBatchTasks()
      batchTasks.value = Array.isArray(tasks) ? tasks : []
    } catch (error: unknown) {
      ElMessage.error(error instanceof Error ? error.message : '加载任务列表失败')
    } finally {
