  ...
  <div v-else-if="ctx.technicalViewData.value">
    <div class="coverage-info">
      ...
      <el-button
        type="primary"
        size="small"
        @click="ctx.batchLocatorVisible.value = true"
    ...
    <div
      v-if="ctx.technicalViewData.value.precondition"
    ...
    >
      ...
      >
        <div style="display: flex; align-items: center; gap: 8px">
          ...
          <el-tag
            v-if="ctx.technicalViewData.value.precondition_steps?.length"
            size="small"
            type="info"
          >
        ...
        <div style="display: flex; gap: 8px">
          <el-button
            type="primary"
            size="small"
            @click="ctx.parsePrecondition"
            :loading="ctx.parsePreconditionLoading.value"
            >AI解析</el-button
          >
      ...
      <el-alert
        :title="`前置条件: ${ctx.technicalViewData.value.precondition}`"
        type="info"
        :closable="false"
        show-icon
      ...
      <el-table
        v-if="ctx.technicalViewData.value.precondition_steps?.length"
        :data="ctx.technicalViewData.value.precondition_steps"
        border
        stripe
        size="small"
        style="width: 100%"
      >
        <el-table-column label="步骤" width="60" align="center">
          <template #default="{ row }"
            ><span>{{ row.step_number }}</span></template
          >
        </el-table-column>
        <el-table-column label="操作" min-width="180">
          <template #default="{ row }"
            ><span>{{ row.action || '-' }}</span></template
          >
        </el-table-column>
        <el-table-column label="操作类型" width="90" align="center">
          <template #default="{ row }">
            <el-tag size="small" :type="ctx.getActionTypeTagType(row.action_type)">{{
              row.action_type || '-'
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="测试数据" min-width="120">
          <template #default="{ row }">
            ...
            <el-button
              ...
              @click="openTestDataEditor(row.step_id ?? row.id, row.action ?? '')"
              >{{ row.input_value ? '编辑' : '添加数据' }}</el-button
            >
          </template>
        </el-table-column>
        <el-table-column label="目标元素" min-width="120">
          <template #default="{ row }"
            ><span>{{ row.target_element || '-' }}</span></template
          >
        </el-table-column>
        <el-table-column label="定位状态" width="90" align="center">
          <template #default="{ row }">
            <el-tag :type="ctx.getLocatorStatusType(row.locator_status)" size="small">{{
              ctx.getLocatorStatusLabel(row.locator_status)
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="定位类型" width="110" align="center">
          <template #default="{ row }">
            <el-tag
              ...
              >{{ getLocatorTypeLabel(row.locator.locator_type) }}</el-tag
            >
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column label="定位值" min-width="200">
          <template #default="{ row }">
            <span
              ...
              >{{ row.locator.locator_value }}</span
            >
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="120" align="center">
          <template #default="{ $index }">
            <el-button size="small" type="danger" text @click="ctx.deletePreconditionStep($index)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <div
      v-if="ctx.isCorrectionMode.value && (ctx.failureReason.value || ctx.aiAnalysisText.value)"
    ...
    >
      <div class="suggestion-header">
        ...
        <el-button
          size="small"
          text
          @click="ctx.showSuggestionPanel.value = !ctx.showSuggestionPanel.value"
        >
          {{ ctx.showSuggestionPanel.value ? '收起' : '展开' }}
        </el-button>
      </div>
      <div v-if="ctx.showSuggestionPanel.value" class="suggestion-body">
        <div v-if="ctx.failureReason.value" class="suggestion-item">
          <strong>失败原因：</strong>
          <p>{{ ctx.failureReason.value }}</p>
        </div>
        <div v-if="ctx.aiAnalysisText.value" class="suggestion-item">
          <strong>AI分析：</strong>
          <p>{{ ctx.aiAnalysisText.value }}</p>
        </div>
      </div>
    </div>

    <el-table
      :data="ctx.technicalViewData.value.steps"
      style="width: 100%; margin-top: 16px"
      border
      stripe
      :row-class-name="ctx.getStepRowClass"
    >
      <el-table-column label="步骤" width="80" align="center">
        <template #default="{ row }"
          ><span class="step-index">{{ row.step_number }}</span></template
        >
      </el-table-column>
      <el-table-column label="操作" min-width="200">
        <template #default="{ row, $index }">
          <div
            v-if="
              ctx.editingCell.value?.stepIndex === $index &&
              ctx.editingCell.value?.field === 'action'
            "
            class="cell-editing"
          >
            ...
            <div class="cell-actions">
              <el-button
                size="small"
                type="success"
                @click="ctx.saveCellEdit"
                :loading="ctx.cellSaving.value"
                >保存</el-button
              >
          ...
          <div
            v-else
            class="cell-display"
            @dblclick="ctx.startCellEdit($index, 'action', row.action)"
          >
            <span>{{ row.action || '-' }}</span>
            <el-icon v-if="ctx.issueType.value !== 'product_bug'" class="edit-icon"
              ><Edit
            /></el-icon>
          </div>
        </template>
      ...
      <el-table-column label="预期结果" min-width="200">
        <template #default="{ row, $index }">
          <div
            v-if="
              ctx.editingCell.value?.stepIndex === $index &&
              ctx.editingCell.value?.field === 'expected_result'
            "
            class="cell-editing"
          >
            <el-input v-model="ctx.editingValue.value" size="small" />
          ...
          <div
            v-else
            class="cell-display"
            @dblclick="ctx.startCellEdit($index, 'expected_result', row.expected_result)"
          >
      ...
      <el-table-column label="定位状态" width="120" align="center">
        <template #default="{ row }">
          <el-tag :type="ctx.getLocatorStatusType(row.locator_status)">{{
            ctx.getLocatorStatusLabel(row.locator_status)
          }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="XPath" min-width="200">
        <template #default="{ row, $index }">
          <div
            v-if="
              ctx.editingCell.value?.stepIndex === $index &&
              ctx.editingCell.value?.field === 'xpath'
            "
            class="cell-editing"
          >
            <el-input v-model="ctx.editingValue.value" size="small" />
          ...
          <div
            v-else
            class="cell-display"
            @dblclick="ctx.startCellEdit($index, 'xpath', row.locator?.xpath)"
          >
      ...
      >
        <template #default="{ row, $index }">
          <el-button
            ...
            type="primary"
            link
            @click="ctx.openAddLocator($index, row)"
            >添加定位</el-button
          >
        </template>
      </el-table-column>
    ...
    <div v-if="editingStepId !== null" class="test-data-editor-section">
      <div class="editor-header-bar">
        <h4>步骤测试数据编辑</h4>
        <el-button size="small" @click="closeTestDataEditor">收起</el-button>
      </div>
      <TestDataEditor :step-id="editingStepId" :step-action="editingStepAction" />
    </div>
  </div>
</template>
...
import TestDataEditor from '@/components/testData/TestDataEditor.vue'

const ctx = useCaseDetail()

const editingStepId = ref<number | null>(null)
const editingStepAction = ref('')

async function openTestDataEditor(stepId: number, action: string): Promise<void> {
  if (editingStepId.value !== null && editingStepId.value !== stepId) {
    try {
      await ElMessageBox.confirm(
        '当前步骤的测试数据编辑尚未保存，切换步骤将丢失未保存的内容。是否继续？',
        '提示',
  ...
  editingStepAction.value = action
}

function closeTestDataEditor(): void {
  editingStepId.value = null
  editingStepAction.value = ''
}
</script>
