    ...
    <template v-else-if="store.projectCases.length > 0">
      <div class="history-case-selector">
        <div class="history-case-toolbar">
          <span class="history-case-stat"
            >共 <strong>{{ store.projectCases.length }}</strong> 条</span
          >
          <el-tag
            v-if="store.selectedHistoryCaseIds.length > 0"
            size="small"
            type="primary"
            effect="dark"
            >已选 {{ store.selectedHistoryCaseIds.length }}</el-tag
          >
          <div class="history-case-actions">
            <el-button
              ...
              :disabled="
                store.projectCases.length === 0 ||
                store.selectedHistoryCaseIds.length >= store.projectCases.length
              "
              >全选</el-button
            >
            <el-button
              ...
              @click.stop.prevent="
                () => {
                  store.selectedHistoryCaseIds = []
                  store._historyCaseUserCleared = true
                }
              "
        ...
        <el-select
          ...
          @change="
            (val: number[]) => {
              if (val.length > 0) store._historyCaseUserCleared = false
              else store._historyCaseUserCleared = true
            }
          "
          multiple
          filterable
        ...
        <div class="history-case-tip">
          <span v-if="store.selectedHistoryCaseIds.length > 0"
            >已选 {{ store.selectedHistoryCaseIds.length }} 条</span
          >
