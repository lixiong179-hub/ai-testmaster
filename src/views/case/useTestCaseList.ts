...
import type BatchLocatorDialog from './BatchLocatorDialog.vue'
...
export function useTestCaseList() {
  ...
  if (Number.isFinite(initProjectId) && initProjectId > 0) {
    selectedProjectId.value = initProjectId
  }
  ...
  const selectedCaseIds = computed(() => selectedRows.value.map((r) => r.id))
  const { exporting, handleExportAll, handleExportSelected } = useCaseExport({
    filteredTestCases,
  ...
  async function fetchTestCases(): Promise<void> {
    ...
    loading.value = true
    try {
      const requestParams: TestCaseListParams = {
        ...filters,
        page: page.value,
        page_size: pageSize.value,
      }
      // 设备类型筛选：undefined/clearable清除表示"全部"不传参，"general"表示"通用"传参让后端匹配null
      if (filters.target_device === undefined) {
        delete requestParams.target_device
      }
      const response: TestCaseListResponse = await caseApi.getList(
      ...
      if (response.stats) {
        ...
        stats.automated = response.stats.automated
        stats.manual = response.stats.manual
        stats.highPriority = response.stats.high_priority
      }
    } catch (error) {
      ElMessage.error(error instanceof Error ? error.message : '加载用例失败')
      testCases.value = []
      total.value = 0
    } finally {
      loading.value = false
  ...
  function resetFilters(): void {
    ...
    filters.created_by = ''
    filters.target_device = undefined
  ...
  async function handleDelete(row: TestCase): Promise<void> {
    try {
      ...
      await caseApi.delete(row.id, row.project_id)
  ...
  function getPriorityType(priority: number): 'danger' | 'warning' | 'success' {
  ...
  function getTypeTagType(type: string): 'primary' | 'success' | 'warning' | 'info' | 'danger' {
    const m: Record<string, string> = {
      ...
      manual: 'info',
      api_automation: 'success',
      performance: 'warning',
      security: 'danger',
    }
    return (m[type] as 'primary' | 'success' | 'warning' | 'info' | 'danger') || 'info'
  }

  function handleQualityAnalysis(row: TestCase): void {
    if (!row.id) {
      ElMessage.warning('请选择具体用例再进入质量分析')
      return
    }
    router.push(`/home/case/quality/${row.id}`)
  }

  function handleBatchCreateTask(): void {
    if (!selectedProjectId.value) {
    ...
    const caseIds = selectedRows.value.map((r) => r.id).join(',')
    router.push({
      path: `/home/task/create/${selectedProjectId.value}`,
  ...
  function handleBatchLocate(): void {
    if (!selectedProjectId.value) {
      ElMessage.warning('请先选择项目')
      return
    }
    ...
    if (!batchLocatorRef.value) {
      ElMessage.warning('批量定位组件未就绪')
      return
    }
    const caseIds = selectedRows.value.map((r) => r.id)
    batchLocatorRef.value.open(caseIds)
  }

  function setBatchLocatorRef(ref: InstanceType<typeof BatchLocatorDialog>): void {
    batchLocatorRef.value = ref
  }
