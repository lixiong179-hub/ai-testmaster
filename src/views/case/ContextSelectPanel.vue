  ...
  <el-card class="step-card">
    <template #header>
      <div class="card-header">
        <span>选择项目与生成上下文</span>
        <el-tag type="info">系统会根据资产自动推荐生成方式</el-tag>
      </div>
    </template>
    <el-form label-width="120px">
      ...
      <ContextHistoryCases />
      <ContextScreenPreview
        ref="screenPreviewRef"
        @preview-screen="(url: string) => emit('preview-screen', url)"
        @test-point-link="handleTestPointLinkResult"
      />
      ...
      <el-form-item>
        <el-button type="primary" @click="$emit('next')"
          >下一步：检查上下文质量<el-icon><ArrowRight /></el-icon
        ></el-button>
...
function getFlowSortEditorRef() {
  return screenPreviewRef.value?.getFlowSortEditorRef?.() ?? null
}
