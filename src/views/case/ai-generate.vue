<template>
  <div class="ai-generate-container">
    ...
    <el-card v-if="currentStep === 0" class="step-card">
      ...
      <el-form label-width="120px">
        <el-form-item label="项目">
          <el-select v-model="formData.project_id" placeholder="请选择项目" style="width: 400px" @change="handleProjectChange">
            <el-option
        ...
        <el-form-item label="需求文档">
          ...
          <el-button type="primary" plain size="small" @click="extractFileContent" :loading="fetchingRequirement" style="margin-left: 10px">
            提取内容
          </el-button>
          <el-button type="success" plain size="small" @click="goToResourceManage" style="margin-left: 10px">
            上传文件
          </el-button>
        </el-form-item>
              ...
              >
                ...
                >
                  <div class="version-option">
                    ...
                    <el-tag
                      v-if="project.parse_status === 'completed'"
                      type="success"
                      size="small"
                    >
                      ✓ 已解析
        ...
        <!-- UI屏幕预览和排序区域 -->
        <el-form-item label="屏幕预览" v-if="selectedUiPrototypeProjectId">
          <el-alert
            v-if="selectedUiPrototypeProject && selectedUiPrototypeProject.parse_status !== 'completed' && showParseWarning"
            :title="selectedUiPrototypeProject.parse_status === 'partial'
          ...
          />
          <div v-if="uiScreens.length > 0" class="ui-screens-container">
            <div class="screens-header">
              <span class="screens-count">共 {{ uiScreens.length }} 个屏幕（拖拽调整顺序）</span>
            </div>
            <div class="ui-screens-grid">
              <div
                v-for="(screen, index) in uiScreens"
                :key="screen.id"
                class="ui-screen-card"
                draggable="true"
                @dragstart="handleUIScreensDragStart($event, index)"
                @dragover.prevent="handleUIScreensDragOver($event, index)"
                @drop="handleUIScreensDrop($event, index)"
                @dragend="handleUIScreensDragEnd"
                :class="{ 'is-dragging': uiScreensDragIndex === index }"
              >
                <div class="screen-order">{{ index + 1 }}</div>
                <div class="screen-drag-handle">
                  <el-icon><Rank /></el-icon>
                </div>
                <div class="screen-image-wrapper" @click="handlePreviewScreenImage(screen)">
                  <img
                    v-if="screenImageUrls[screen.id]"
                    :src="screenImageUrls[screen.id]"
                    :alt="screen.screen_name"
                    class="screen-thumbnail"
                  />
                  <div v-else class="screen-thumbnail-placeholder">
                    <el-icon :size="24"><Picture /></el-icon>
                  </div>
                  <div class="screen-preview-overlay">
                    <el-icon><View /></el-icon>
                  </div>
                </div>
                <div class="screen-info">
                  <div class="screen-name">{{ screen.screen_name }}</div>
                  <div v-if="screen.summary" class="screen-desc">{{ screen.summary }}</div>
                  <div class="screen-meta">
                    <el-tag
                      v-if="screen.parse_status"
                      :type="screen.parse_status === 'completed' ? 'success' : screen.parse_status === 'failed' ? 'danger' : 'warning'"
                      size="small"
                    >
                      {{ screen.parse_status === 'completed' ? '已解析' : screen.parse_status === 'failed' ? '解析失败' : screen.parse_status === 'running' ? '解析中' : '待解析' }}
                    </el-tag>
                    <span v-if="screen.element_count" class="screen-elements">{{ screen.element_count }}个元素</span>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="uiScreens.length > 1" class="screens-footer">
              <el-button 
                type="primary" 
                @click="saveUIScreenOrder" 
                :loading="savingUIScreenOrder"
              >
          ...
          <div class="test-point-selector" :class="{ 'has-data': testPointTotal > 0 }">
            ...
            <div class="tp-toolbar" v-if="testPointTotal > 0 || formData.test_point_ids.length > 0">
              <div class="tp-toolbar-left">
                <span class="tp-stat" v-if="testPointTotal > 0">共 <strong>{{ testPointTotal }}</strong> 条</span>
                <el-tag size="small" type="primary" effect="dark" v-if="formData.test_point_ids.length > 0">
                  已选 {{ formData.test_point_ids.length }}
                </el-tag>
              </div>
              <div class="tp-toolbar-right">
                <el-button link type="primary" size="small" @mousedown.prevent @click.stop.prevent="selectAllTestPoints"
                  :disabled="testPointAllIds.length === 0">
                  全选
                </el-button>
                <el-button link type="danger" size="small" @mousedown.prevent @click.stop.prevent="deselectAllTestPoints"
            ...
            >
              ...
              <template #header>
                <div class="tp-d-header" @mousedown.prevent>
                  ...
                  <div class="tp-d-info">
                    <span>{{ testPointPage }}/{{ Math.ceil(testPointTotal / testPointPageSize) || 1 }}</span>
                    <el-button link size="small" type="primary" @mousedown.prevent @click.stop.prevent="selectCurrentPageAll">
                      本页全选
                    </el-button>
                  </div>
                </div>
              ...
              >
                <div class="tp-item" @mousedown.prevent @click.stop>
                  <label class="tp-check" @mousedown.prevent @click.stop>
                    <input type="checkbox" :checked="formData.test_point_ids.includes(point.id)"
                      @click.stop
                      @change="(e: Event) => { (e.target as HTMLInputElement).checked ? addTestPoint(point.id) : removeTestPoint(point.id); }" />
                  </label>
                  <div class="tp-content">
                    <div class="tp-row1">
                      ...
                      <el-tag size="small" :type="getPriorityType(point.priority)" round class="tp-pri">
            ...
            <transition name="el-fade-in-linear">
              <div class="tp-chips" v-if="formData.test_point_ids.length > 0">
                ...
                <el-popover v-if="formData.test_point_ids.length > 6" placement="bottom-start" :width="280" trigger="hover">
                  <template #reference>
                    <el-tag effect="dark" type="info" round size="small">+{{ formData.test_point_ids.length - 6 }}</el-tag>
                  </template>
                  <div style="max-height:200px;overflow-y:auto;padding:4px 0;">
                    <el-tag v-for="id in formData.test_point_ids.slice(6)" :key="id" closable effect="dark" :type="getSelectedTagType(id)" size="small" style="margin:2px" @close="removeTestPoint(id)">
        ...
        <el-form-item label="上下文预览" v-if="contextPreview">
          <div class="context-preview">
            <el-alert :title="contextPreview.title" :type="contextPreview.type" show-icon :closable="false">
              <template #default>
                <div v-if="contextPreview.requirement">需求文档: {{ contextPreview.requirement }}</div>
                <div v-if="contextPreview.ui">UI原型: {{ contextPreview.ui }}</div>
                <div v-if="contextPreview.uiSpecs">已解析UI规格: {{ contextPreview.uiSpecs }}个页面</div>
      ...
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item label="待生成范围">
          <div class="scope-preview">
            <div class="scope-stats">
              ...
              <el-statistic title="需求文档" :value="formData.requirement_file_ids.length" class="stat-docs" />
              <el-statistic title="UI原型" :value="formData.ui_screen_ids.length || formData.ui_file_ids.length" class="stat-ui" />
            </div>
            <div class="scope-testpoints" v-if="selectedTestPointsForDisplay.length > 0">
              <div class="st-list">
                <div
                  v-for="tp in selectedTestPointsForDisplay"
                  :key="tp.id"
                  class="st-item"
                >
                  ...
                  <el-tag size="small" :type="getPriorityType(tp.priority)" round>{{ getPriorityLabel(tp.priority) }}</el-tag>
                </div>
              </div>
        ...
        <el-form-item label="用例类型">
          <div class="case-type-group">
            <el-select v-model="formData.case_type" placeholder="请选择用例类型（不选择则由AI智能判断）" style="width: 180px;" @change="handleCaseTypeChange">
            ...
            <div class="exec-mode-group" v-if="formData.case_type === 'ui_automation'">
              ...
              <el-radio-group v-model="formData.exec_mode" size="small">
                <el-radio-button value="all">
                  全部自动标注
                  <el-tooltip content="AI根据每个用例内容自动判断是UI自动化还是手工测试" placement="top">
        ...
        <el-form-item>
          <el-button type="primary" @click="handleGenerate" :loading="generating" :disabled="canGenerate === false">
    ...
    <el-card v-if="generatedCases.length > 0 && !generating" class="result-card">
      <template #header>
        <div class="card-header">
          <span>生成结果 <el-tag size="small" type="info">{{ currentCaseIndex + 1 }}/{{ generatedCases.length }}</el-tag></span>
          <div class="result-actions">
            ...
            <el-button v-if="!isEditingResult" type="primary" size="small" @click="handleSaveCase" :loading="saving">
      ...
      <div class="case-nav-bar" v-if="generatedCases.length > 1 && !isEditingResult">
        ...
        <button class="case-nav-btn" :disabled="currentCaseIndex >= generatedCases.length - 1" @click="currentCaseIndex++">
          下一条 ›
        </button>
        <div class="case-batch-actions">
          <el-button type="success" size="small" @click="saveAllCases" :loading="saving" :disabled="generatedCases.every(c => c._error || c._saved)">
      ...
      <!-- 错误提示（单条失败） -->
      <el-alert v-if="viewingCase?._error" :title="'生成失败: ' + viewingCase._error" type="error" :closable="false" show-icon style="margin-bottom:12px;" />
      ...
      <template v-if="!isEditingResult && viewingCase">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="前置条件" :span="2">{{ viewingCase.precondition || '-' }}</el-descriptions-item>
        </el-descriptions>
      </template>
      ...
      <template v-else-if="isEditingResult">
        <div class="edit-form-grid">
          ...
          <div class="edit-field full-width">
            <label>前置条件</label>
            <el-input v-model="editingCase.precondition" type="textarea" :rows="3" placeholder="请输入前置条件" />
          </div>
        </div>
      </template>

      <!-- 测试数据展示 -->
      <div v-if="viewingCase?.test_data && Object.keys(viewingCase.test_data).length > 0 && !isEditingResult" class="test-data-section">
        <h3>测试数据</h3>
        <el-row :gutter="20">
      ...
      <div style="margin-top: 20px;">
        <h3>
          测试步骤
          <el-button v-if="isEditingResult" type="primary" size="small" @click="addEditStep" style="margin-left: 10px;">+ 添加步骤</el-button>
        </h3>
        <template v-if="!isEditingResult && viewingCase">
          <el-timeline>
            <el-timeline-item
            ...
            >
              <el-card>
                <div class="step-content">
                  ...
                  <div class="step-expected"><strong>预期结果:</strong> {{ cleanExpectedResult(step.expected_result) }}</div>
                </div>
              </el-card>
            </el-timeline-item>
          </el-timeline>
          ...
          <div class="editable-steps-list">
            <div v-for="(step, index) in editingCase.steps" :key="index" class="editable-step-card">
              <div class="step-card-header">
                ...
                <el-button type="danger" size="small" @click="removeEditStep(Number(index))" :disabled="editingCase.steps.length <= 1">删除</el-button>
              </div>
            ...
            <div v-if="editingCase.steps.length === 0" class="empty-steps-hint"><p>暂无步骤，点击上方"添加步骤"</p></div>
          </div>
        </template>
      </div>
      ...
      <div v-if="viewingCase?.expected_result && !isEditingResult" class="expected-result-section">
        <h3>总体预期结果</h3>
        <el-alert :title="cleanExpectedResult(viewingCase.expected_result)" type="success" :closable="false" show-icon />
      </div>
      <div v-else-if="isEditingResult" class="expected-result-section">
        <h3>总体预期结果</h3>
        <el-input v-model="editingCase.expected_result" type="textarea" :rows="2" placeholder="请输入总体预期结果" />
      </div>
      ...
      <div class="result-actions-bottom" v-if="!isEditingResult">
        ...
        <el-button type="success" @click="handleContinueGenerate" :loading="generating">基于此用例继续生成</el-button>
      </div>
    </el-card>
...
import { ArrowLeft, ArrowRight, MagicStick, Close, Check, Refresh, Upload, Search, InfoFilled, Edit, Rank, View, Picture } from '@element-plus/icons-vue';
...
const uiScreensDragIndex = ref<number | null>(null);
const savingUIScreenOrder = ref(false);
...
const selectedTestPointsForDisplay = computed(() => {
  ...
  return ids.map(id => {
    const tp = testPoints.value.find(p => p.id === id) || formData.test_points_data.find((p: any) => p.id === id);
...
const canGenerate = computed(() => {
  return formData.test_point_ids.length > 0
...
const generateButtonLabel = computed(() => {
  ...
  if (formData.requirement_file_ids.length > 0) return `${formData.requirement_file_ids.length} 份需求`;
...
const getProjects = async () => {
  try {
    const response: ApiResponse<{ items: Project[] }> = await request.get('/api/v1/project/list');
...
// 加载测试点（根据需求文档关联的测试点）
const loadTestPoints = async (page: number = 1, append: boolean = false, refreshKey: boolean = true) => {
  ...
  try {
    ...
    if (apiData) {
      ...
      if (testPointAllIds.value.length < testPointTotal.value && page === 1 && testPointTotal.value <= 200) {
...
const loadUIPrototypeProjects = async () => {
  ...
  try {
    ...
    if (response) {
      uiPrototypeProjects.value = Array.isArray(response) ? response : (response as any)?.data?.items || [];
    }
  } catch (error) {
...
const loadUIScreens = async (uiPrototypeProjectId: number) => {
  ...
  try {
    ...
    if (response?.items) {
      uiScreens.value = response.items.sort((a, b) => (a.screen_order || 0) - (b.screen_order || 0));
      loadScreenImages();
    }
  } catch (error) {
  ...
  for (const screen of uiScreens.value) {
    if (screen.id && screen.original_file_path && !screenImageUrls.value[screen.id]) {
      try {
        ...
        const res: any = await request.get(`/api/v1/file/preview-screen/${screen.id}`, {
          responseType: 'blob',
...
const handlePreviewScreenImage = (screen: UIScreen) => {
  if (screen.id && screenImageUrls.value[screen.id]) {
    ...
    ElMessage.warning('图片暂未加载，请稍后再试');
  }
};
  ...
  try {
    ...
    if (response?.code === 200) {
      ...
      contextPreview.value = {
        ...
        ui: formData.ui_screen_ids.length > 0 ? `${formData.ui_screen_ids.length} 个屏幕` : (formData.ui_file_ids.length > 0 ? `${formData.ui_file_ids.length} 个文件` : ''),
  ...
  try {
    const apiData = {
      ...
      context: {
        ...
        test_points: formData.test_point_ids.length > 0 ? [] : (contextPreview.value?.test_points || [])
      }
    };

    const response = await caseApi.aiGenerateCaseEnhanced(apiData as Parameters<typeof caseApi.aiGenerateCaseEnhanced>[0]);
...
const handleGenerate = async () => {
  ...
  const targetPoints = formData.test_point_ids.length > 0
    ? formData.test_point_ids
  ...
  try {
    ...
    if (formData.project_id) {
      const contextResponse: ApiResponse<{
        ...
        project_id: Number(formData.project_id),
        requirement_file_ids: formData.requirement_file_ids.length > 0 ? formData.requirement_file_ids : undefined,
    ...
    for (let i = 0; i < targetPoints.length; i++) {
      ...
      try {
        ...
        console.log(`[AI生成] 第${i+1}/${targetPoints.length}条 - tpId:${tpId}, tpLabel:${tpLabel}`);
        console.log(`[AI生成] tpInfo详情:`, JSON.stringify(tpInfo, null, 2));
        console.log(`[AI生成] apiData.context.current_test_point:`, apiData.context.current_test_point);
    ...
    if (failCount === 0) {
      ...
      ElMessage.warning(`生成完成：${generatedCases.value.length - failCount} 成功，${failCount} 失败`);
    }
  } catch (error: any) {
    progress.value = 100;
    progressText.value = '生成失败';
...
const handleSaveCase = async () => {
  ...
  try {
    ...
    await caseApi.createCase({
      ...
      steps: stepsPayload.length > 0 ? stepsPayload : [{ step: '1', action: '执行测试', param: '预期结果正常', expected_result: '预期结果正常', action_type: 'verify', input_value: '', target_element: '' }],
...
const removeEditStep = (index: number) => {
  if (editingCase.value.steps.length > 1) {
    ...
    editingCase.value.steps.forEach((s: any, i: number) => { s.step = i + 1; });
  }
};
...
watch([() => formData.requirement_file_ids, () => formData.ui_file_ids], async ([newReqIds, newUiIds], [oldReqIds, oldUiIds]) => {
...
// ========== UI 屏幕拖拽排序功能 ==========
const handleUIScreensDragStart = (event: DragEvent, index: number) => {
  uiScreensDragIndex.value = index;
  event.dataTransfer!.effectAllowed = 'move';
  event.preventDefault();
};

const handleUIScreensDragOver = (event: DragEvent, _index: number) => {
  event.preventDefault();
};

const handleUIScreensDrop = (event: DragEvent, targetIndex: number) => {
  event.preventDefault();
  
  if (uiScreensDragIndex.value === null || uiScreensDragIndex.value === targetIndex) {
    uiScreensDragIndex.value = null;
    return;
  }
  
  const sourceIndex = uiScreensDragIndex.value;
  
  const [removed] = uiScreens.value.splice(sourceIndex, 1);
  uiScreens.value.splice(targetIndex, 0, removed);
  
  uiScreensDragIndex.value = null;
};

const handleUIScreensDragEnd = () => {
  uiScreensDragIndex.value = null;
};

// 保存 UI 屏幕排序
const saveUIScreenOrder = async () => {
  savingUIScreenOrder.value = true;
  try {
    for (let i = 0; i < uiScreens.value.length; i++) {
      const screen = uiScreens.value[i];
      await uiPrototypeApi.updateUIScreenOrder(screen.id, i + 1);
    }
    ...
    savingUIScreenOrder.value = false;
  }
};
</script>
...
.screen-image-wrapper:hover .screen-thumbnail {
...
.tp-d-search input {
  ...
  font-size: 13px; width: 100%; color: #606266;
}

.tp-d-search input::placeholder { color: #c0c4cc; }
...
.tp-d-footer .el-pagination { justify-content: center; }
.tp-d-footer .el-pagination .el-pager li {
  min-width: 28px; height: 28px; line-height: 28px;
  font-size: 12px; border-radius: 4px; margin: 0 2px;
}
