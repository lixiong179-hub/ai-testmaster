...
watch(
    [
        () => store.formData.requirement_file_ids,
        () => store.formData.ui_file_ids,
    ],
    async ([newReqIds, newUiIds], [oldReqIds, oldUiIds]) => {
        const reqChanged =
            JSON.stringify(newReqIds || []) !== JSON.stringify(oldReqIds || [])
        const uiChanged =
            JSON.stringify(newUiIds || []) !== JSON.stringify(oldUiIds || [])

        if ((reqChanged || uiChanged) && store.formData.project_id) {
            store.handleSourceFileChange()
            if (
                (newReqIds && newReqIds.length > 0) ||
                (newUiIds && newUiIds.length > 0)
            ) {
                await store.loadTestPoints(1)
            }
        }
