...
const QUICK_MODE_STORAGE_KEY = 'flow-editor-quick-mode'
...
export const useFlowSortStore = defineStore('flowSort', () => {
  ...
  let quickModeInitial = true
  try {
    const stored = localStorage.getItem(QUICK_MODE_STORAGE_KEY)
    if (stored !== null) quickModeInitial = stored !== 'false'
  } catch {
    /* localStorage unavailable, fallback to default */
  }
  const quickMode = ref<boolean>(quickModeInitial)

  /**
   * 切换快速/编辑模式并持久化偏好。
   * 快速模式(quickMode=true): 不自动生成连线, 走 linear 生成, 跳过流程校验。
   * 编辑模式(quickMode=false): 自动生成连线, 走 graph 生成, 执行流程校验。
   * localStorage 不可用时仅存内存, 不阻断页面渲染。
   */
  function setQuickMode(val: boolean): void {
    quickMode.value = val
    try {
      localStorage.setItem(QUICK_MODE_STORAGE_KEY, String(val))
    } catch {
      /* silent */
    }
  }
  ...
  function persistToStorage() {
    try {
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ ...sortData.value, _project_id: projectId.value })
      )
    } catch {
      /* silent */
    }
  ...
  function setProjectId(id: number) {
    projectId.value = id
    if (saveStatus.value === 'unsaved') triggerAutoSave()
  }
  ...
  async function performSave(retryOn5xx = true): Promise<void> {
    ...
    try {
      const flowData: ProjectFlowData = {
        nodes: mapNodesToFlowData(nodes.value),
        edges: mapEdgesToFlowData(edges.value),
      ...
      if (isRetryable && retryOn5xx) {
        ElMessage.warning(
          status === null ? '保存超时，3秒后自动重试...' : '服务器繁忙，3秒后自动重试保存...'
        )
