...
export const useCaseStore = defineStore('case', {
  ...
  actions: {
    ...
    async fetchTestCases(projectId: number, params?: {
      module?: string;
      priority?: number;
    ...
    async updateTestCase(caseId: number, data: Partial<TestCase>) {
      try {
        const response = await testCaseApi.updateCase(caseId, data as Record<string, unknown> as Parameters<typeof testCaseApi.updateCase>[1]);
        await this.fetchTestCases(this.currentProjectId);
        return response;
      } catch (error) {
    ...
    async copyTestCase(caseId: number) {
      try {
        ...
        const { title, module, priority, case_type, precondition, steps, expected_result } = copyData;
