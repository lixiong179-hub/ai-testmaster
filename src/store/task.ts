...
function normalizeTaskResults(data: TaskResultsPayload): TestResult[] {
...
export const useTaskStore = defineStore('task', {
  ...
  getters: {
    ...
    taskStatusColor(): (status: number) => TagType {
      return (status: number): TagType => {
    ...
    execStatusText(): (status: number) => string {
      return (status: number): string => {
        const statusMap: Record<number, string> = {
          ...
          [ExecutionStatus.BLOCKED]: '阻塞',
        }
        return statusMap[status] || '未知状态'
      }
    },
    execStatusColor(): (status: number) => TagType {
      return (status: number): TagType => {
        const colorMap: Record<number, TagType> = {
          [ExecutionStatus.PENDING]: 'info',
          [ExecutionStatus.PASSED]: 'success',
          [ExecutionStatus.FAILED]: 'danger',
          [ExecutionStatus.BLOCKED]: 'warning',
        }
        return colorMap[status] || 'info'
      }
    },
    logStatusText(): (status: number) => string {
      return (status: number): string => {
        const m: Record<number, string> = { 0: '信息', 1: '成功', 2: '错误', 3: '警告' }
        return m[status] || '未知'
      }
    },
    logStatusColor(): (status: number) => TagType {
      return (status: number): TagType => {
        const m: Record<number, TagType> = { 0: 'info', 1: 'success', 2: 'danger', 3: 'warning' }
  ...
  actions: {
    async fetchTaskList(params?: {
      ...
      try {
        ...
        this.taskList = items
        return { items: this.taskList, total: data?.total || this.taskList.length }
      } catch (error: unknown) {
    ...
    async fetchTaskResults(taskId: number, projectId?: number) {
      ...
      try {
        ...
        const data = extractTaskResponseData<TaskResultsPayload>(response)
        this.taskResults = normalizeTaskResults(data)
    ...
    async startTask(
      taskId: number,
      projectId?: number,
      executionMode?: 'preprocess' | 'realtime' | 'smart' | 'mobile_realtime' | 'mobile_smart',
      mobileDeviceId?: string
    ) {
      this.loading = true
      this.error = null
      try {
        const response = await testTaskApi.startTask(
          taskId,
          projectId,
          executionMode,
          mobileDeviceId
    ...
    addExecutionLog(log: ExecutionLog) {
      ...
      if (this.executionLogs.length > 1000) this.executionLogs = this.executionLogs.slice(-500)
    },
